...
Lemma normal_or_adversary: forall (i:D), 
  ( i \in Normal) \/ (i \in Adversary).
Proof.
intros. apply /orP. rewrite -in_setU.
assert (Adversary = ~: Normal).
{ apply eq_finset. unfold eqfun. intros. rewrite !inE.
  by destruct (corrupt x == true).
...
Lemma cardD_sum: 
  #|D| =( #|Normal| + #|Adversary|)%N.
Proof.
rewrite -cardsUI.
assert (#|Normal :&: Adversary| = 0%N).
...
Lemma size_normal_gt_0: 
  F_totally_bounded ->
  (0 < F + 1 <= #|D|)%N -> 
  (0 < size (enum Normal))%N.
Proof.
intros. 
assert ((0 < F+1)%N /\ (F+1 <= #|D|)%N).
{ by apply /andP. } destruct H1. rewrite -cardE.
rewrite cardD_sum in H2. 
apply (@add_max_bound #|Normal| #|Adversary| F) in H2.
+ by [].
...
Lemma normal_set_to_enum: forall (i:D),
  i \in Normal -> i \in enum Normal.
Proof.
intros.
assert ([set x | x \in enum Normal] = Normal).
{ apply set_enum. } rewrite -H0 in H.
by rewrite inE in H.
Qed.


Lemma nth_normal: forall (i:nat), 
  (i < size (enum Normal))%N ->  
  nth key (enum Normal) i \in Normal.
Proof.
intros.
assert ( nth key (enum Normal) i \in enum Normal ->
          nth key (enum Normal) i \in Normal).
{ apply (@in_enum_Normal (nth key (enum Normal) i) Normal). }
by apply H0, mem_nth.
Qed.


Lemma x_bound: forall (i:D), 
  i \in Normal ->  
  forall t : nat, (m t <= x t i <= M t)%Re.
Proof.
intros. 
...
**)
split.
+ assert (m t = -(bigmaxr 0 ((map (fun i:D => -(x t i))) (enum Normal)))).
  { by rewrite /m. } rewrite H0.
  assert ( x t i = (- (- (x t i)))). 
  { rewrite -!RoppE. by rewrite Ropp_involutive. } rewrite H1.
  apply Ropp_le_contravar. rewrite -RoppE.
  assert ( nth 0%Re ((map (fun i:D => -(x t i))) (enum Normal)) 
            (index i (enum Normal))= 
                     (fun i:D => - (x t i)) (nth key (enum Normal) 
            (index i (enum Normal)))).
  { apply (@nth_map D key R 0%Re (fun i:D => -(x t i)) (index i (enum Normal)) (enum Normal)).
    rewrite index_mem. by apply normal_set_to_enum.
  }
  assert ((- x t i)%Re  = (fun i : D => - x t i) (nth key (enum Normal)
          (index i (enum Normal)))).
  { assert ( i  = nth key (enum Normal) (index i (enum Normal))).
    { symmetry. by apply nth_index, normal_set_to_enum. }
    by rewrite [in LHS]H3.
  } rewrite H3. rewrite -H2.
  apply /RleP. 
  apply (@bigmaxr_ler _ 0%Re [seq - x t i0 | i0 <- enum Normal] (index i (enum Normal)) ). 
  rewrite size_map. rewrite index_mem. by apply normal_set_to_enum.
+ assert (M t = bigmaxr 0 (map (x t) (enum Normal))).
...
Lemma order_m: 
  F_totally_bounded ->
  (0 < F + 1 <= #|D|)%N -> 
  forall t : nat, (m t <= M t)%Re.
Proof.
intros. rewrite /m /M.
apply Rle_trans with 
  (-(nth 0%Re [seq (- x t i)%Ri | i <- enum Normal] (size (enum Normal)).-1))%Re.
+ apply Ropp_le_contravar. apply /RleP.
  apply (@bigmaxr_ler _ 0%Re [seq - x t i0 | i0 <- enum Normal] (size (enum Normal)).-1).
  rewrite size_map. rewrite ltn_predL. by apply size_normal_gt_0.
+ assert ((- nth 0 [seq (- x t i)%Ri | i <- enum Normal]
              (size (enum Normal)).-1)%Re = 
          nth 0 [seq x t i | i <- enum Normal]
              (size (enum Normal)).-1).
  { rewrite -(@nth_map R 0%Re R 0%Re (fun x:R => (- x)%Re) 
          (size (enum Normal)).-1 [seq (- x t i)%Ri | i <- enum Normal]).
    + assert ([seq (- x)%Re | x <- [seq - x t i | i <- enum Normal]] = 
                [seq x t i | i <- enum Normal]).
      { rewrite -map_comp.
        apply eq_in_map. 
        rewrite /eqfun //=. 
        assert ( (forall i:D, i \in (enum Normal) -> 
                  (- ( - (x t i))%Ri)%Re = x t i) <-> 
                  {in enum Normal,
                    forall x : D,
                    (- (- definitions.x t x)%Ri)%Re = definitions.x t x}).
...
Lemma interval_bound:
  ...
  (0 < F + 1 <= #|D|)%N -> 
  forall t : nat,
  (m 0 <= m t)%Re /\ (M t <= M 0)%Re.
Proof.
intros. induction t.
  - nra.
  - destruct IHt as [mIHt MIHt].
    split.
    * apply Rle_trans with (m t)%Re.
      ++ apply mIHt.
      ++ assert (m t.+1 = -(bigmaxr 0 ((map (fun i:D => -(x t.+1 i))) (enum Normal)))).
         { by rewrite /m. } rewrite H2. clear H2.
         assert ( m t = (-(-(m t)))%Re). { by rewrite Ropp_involutive. }
         ...
         - intros. rewrite size_map in H3.
           assert ( nth 0%Re ((map (fun i:D => -(x t.+1 i))) (enum Normal)) i= 
                      (fun i:D => - (x t.+1 i)) (nth key (enum Normal) i)).
           { by apply (@nth_map D key R 0%Re (fun i:D => -(x t.+1 i)) i (enum Normal)). }
           rewrite H4. apply Ropp_ge_le_contravar. apply Rle_ge.
           assert (forall (i:D) (t:nat), F_total_malicious ->
                    wts_well_behaved ->
                    i \in Normal ->
                    ((x (t+1) i <= M t)%Re /\ (m t <= x (t+1) i)%Re)).
           { apply lem_1. }
           specialize (H5 (nth key (enum Normal) i)).
           specialize (H5 t H H0).
           assert (nth key (enum Normal) i \in Normal). 
           { apply nth_normal, H3. }
           specialize (H5 H6). rewrite -addn1. destruct H5.
           apply H7.
    * apply Rle_trans with (M t)%Re.
      ++ assert (M t.+1 = bigmaxr 0 (map (x t.+1) (enum Normal))).
         { by rewrite /M. } rewrite H2. clear H2.
         apply bigmax_le.
         - rewrite size_map. destruct H. by apply size_normal_gt_0.
         - intros. rewrite size_map in H2.
           assert ( [seq x t.+1 i | i <- enum Normal]`_i = 
                      (fun i:D => (x t.+1 i)) (nth key (enum Normal) i)).
           ...
           assert (forall (i:D) (t:nat), F_total_malicious ->
                    ...
                    ((x (t+1) i <= M t)%Re /\ (m t <= x (t+1) i)%Re)).
           { apply lem_1. }
           specialize (H4 (nth key (enum Normal) i)).
           specialize (H4 t H H0).
           assert (nth key (enum Normal) i \in Normal). 
           { apply nth_normal, H2. }
...
Lemma A_M_exists:
  ...
  wts_well_behaved ->
  (0 < F + 1 <= #|D|)%N ->
  (forall t:nat, (M t.+1 <= M t)%Re) -> 
  exists A_M: R, is_lim_seq M A_M.
Proof.
intros. 
...
apply H3 with (m 0%N).
+ apply H2.
+ intros. apply Rle_trans with (m t).
...
Definition X_M_t_e_i (e_i: R) (A_M:R) (t:nat):=
  [set i:D | Rgt_dec (x t i) (A_M - e_i)%Re].

(** Set of all normal and malicious nodes 
  that have values smaller than A_m + e_i 
**)
Definition X_m_t_e_i (e_i: R) (A_m :R) (t:nat) :=
  [set i:D | Rlt_dec (x t i) (A_m + e_i)].


Lemma X_M_X_m_disjoint:
  forall (t:nat) (A_M A_m :R) (eps_0:posreal),
  (A_M - eps_0 > A_m + eps_0)%Re ->
  [disjoint (X_M_t_e_i eps_0 A_M t) & (X_m_t_e_i eps_0 A_m t)].
Proof.
...
destruct (Rgt_dec (x t i) (A_M - eps_0)).
+ simpl. 
  destruct (Rlt_dec (x t i) (A_m + eps_0)).
  - simpl. left.
    assert ((x t i > A_m + eps_0)%Re).
    { apply Rgt_trans with (A_M - eps_0)%Re; by []. }
    assert ((x t i > A_m + eps_0)%Re -> 
            ~(x t i < A_m + eps_0)%Re). { nra. }
    specialize (H1 H0).
    contradiction.
  - simpl. by right.
+ simpl. by left.
...
Lemma each_elem_disjoint:
  forall (t:nat) (A_M A_m :R) (eps_0:posreal),
  (A_M - eps_0 > A_m + eps_0)%Re -> 
  (0 < #|X_m_t_e_i eps_0 A_m t|)%N ->
  [disjoint
   [set nth key (enum (X_m_t_e_i eps_0 A_m t)) 0]
    & X_M_t_e_i eps_0 A_M t].
Proof.
...
Lemma X_M_mem:
  ...
  (0 < #|X_m_t_e_i eps_0 A_m t_eps|)%N -> 
  X_M_t_e_i eps_0 A_M t_eps \proper Vertex.
Proof.
intros.
rewrite properEcard.
apply /andP. split.
...
- rewrite cardsCs.
  ...
  * rewrite card_gt0. apply /set0Pn.
    exists (nth key (enum (X_m_t_e_i eps_0 A_m t_eps)) 0%N).
...
- rewrite cardsCs.
  ...
  * rewrite card_gt0. apply /set0Pn.
    ...
    apply single_element_disjoint.
    ++ by [].
    ++ by apply (@X_M_X_m_disjoint t_eps A_M A_m eps_0).
  * apply H.
Qed.
...
+ intros.
  assert ( nth 0%Re ((map (fun i:D => (x t.+1 i))) (enum Normal)) i= 
               (fun i:D => (x t.+1 i)) (nth key (enum Normal) i)).
  { apply (@nth_map D key R 0%Re (fun i:D => (x t.+1 i)) i (enum Normal)). 
...
+ intros.
  ...
  assert (forall (i:D) (t:nat), F_total_malicious ->
                    ...
                    ((x (t+1) i <= M t)%Re /\ (m t <= x (t+1) i)%Re)).
  { apply lem_1. }
  specialize (H6 (nth key (enum Normal) i)).
  specialize (H6 t H H0).
  assert (nth key (enum Normal) i \in Normal). 
...
Theorem sandwich_func: forall (u v : nat -> R),
(exists L:Rbar,  
   forall (i:D), i \in Normal ->
   (forall t:nat, ((u t) <= (x t i) <= (v t))%Re) /\ 
...
Lemma A_m_lt_A_M: forall (A_m A_M: R),
   F_totally_bounded ->
  (0 < F + 1 <= #|D|)%N ->
  is_lim_seq (fun t:nat => m t) A_m ->
  is_lim_seq (fun t:nat => M t) A_M ->
  A_m <> A_M ->
  (A_m < A_M)%Re.
Proof.
intros.
assert ((forall t:nat, (m t <= M t)%Re) ->
        is_lim_seq (fun t: nat => m t) A_m -> 
        is_lim_seq (fun t: nat => M t) A_M ->
        (A_m <= A_M)%Re).
{ apply is_lim_seq_le. } 
...
Lemma sandwich_x: forall (A_M A_m:R),
  is_lim_seq (fun t:nat => m t ) A_m ->
  is_lim_seq (fun t:nat => M t) A_M ->
  A_M = A_m -> 
  exists L : Rbar,
    forall i : D,
    i \in Normal -> is_lim_seq (x^~ i) L.
Proof.
intros. apply sandwich_func with (fun t:nat => m t) (fun t:nat => M t) .
...
Lemma M_A_M_lower_bound: forall (A_M : R),
  F_total_malicious ->
  wts_well_behaved ->
  (0 < F + 1 <= #|D|)%N ->
  is_lim_seq (fun t:nat => M t) A_M ->
  forall t:nat, (A_M <= M t)%Re.
Proof.
intros.
apply (is_lim_seq_decr_compare (fun t:nat => M t) A_M).
+ apply H2. 
...
Lemma m_A_m_lower_bound: forall (A_m : R),
  ...
  (0 < F + 1 <= #|D|)%N ->
  is_lim_seq (fun t:nat => m t) A_m ->
  forall t:nat, (m t <= A_m)%Re.
Proof.
intros.
apply (is_lim_seq_incr_compare (fun t:nat => m t) A_m).
+ apply H2. 
...
Lemma max_bound (A_M:R):
  F_totally_bounded ->
  (0 < F + 1 <= #|D|)%N ->
  (exists t:nat , forall (i:D), 
    i \in Normal -> (x t i < A_M)%Re) ->
   exists t:nat, (M t < A_M)%Re.
Proof.
intros. destruct H1 as [t H1]. exists t.
rewrite /M. apply bigmax_lt.
+ rewrite size_map. destruct H. by apply size_normal_gt_0.
+ intros. rewrite size_map in H2.
  specialize (H1 (nth key (enum Normal) i)).
  assert (nth key (enum Normal) i \in Normal ).
  { by apply nth_normal. } specialize (H1 H3).
  assert (x t (nth key (enum Normal) i) = 
          [seq x t i | i <- enum Normal]`_i ).
  { assert ( [seq x t i | i <- enum Normal]`_i = 
                 (fun i:D => (x t i)) (nth key (enum Normal) i)).
    { by apply nth_map. } by rewrite H4.
  } rewrite -H4. by apply H1.
Qed.

Lemma min_bound (A_m:R):
   F_totally_bounded ->
  (0 < F + 1 <= #|D|)%N ->
  (exists t:nat , forall (i:D), 
    i \in Normal -> (x t i > A_m)%Re) ->
...
+ intros. rewrite size_map in H3.
  ...
  { assert ( [seq - (x t i) | i <- enum Normal]`_i = 
                 (fun i:D => - (x t i)) (nth key (enum Normal) i)).
    { by apply (@nth_map D key R 0%Re (fun i:D => - (x t i)) i (enum Normal)) . }
    by rewrite H5.
...
Lemma X_M_normal_exists (t_eps: nat) (A_M: R) (eps_0:posreal): 
   ...
   wts_well_behaved ->
  (0 < F + 1 <= #|D|)%N ->
   is_lim_seq [eta M] A_M ->
    exists i:D, i \in (X_M_t_e_i eps_0 A_M t_eps) /\ i \in Normal.
Proof.
intros. 
assert (M t_eps \in map (fun i:D => x t_eps i) (enum Normal)).
{ rewrite /M. 
  apply (@bigmaxr_mem _ 0%Re (map (fun i:D => x t_eps i) (enum Normal))).
  rewrite size_map. destruct H. by apply size_normal_gt_0.
} 
assert (exists2 i:D, i \in (enum Normal) &
            (M t_eps = (fun i:D => x t_eps i) i)).
{ by apply /mapP. }
destruct H4 as [i H4]. exists i.
split.
+ rewrite inE. rewrite -H5.
  destruct (Rgt_dec (M t_eps) (A_M - eps_0)).
  - by simpl.
  - simpl. 
    assert ((A_M - eps_0 < M t_eps)%Re).
...
split.
+ rewrite inE.
  assert (m t_eps = x t_eps i). 
  { rewrite -[LHS]Ropp_involutive. rewrite !RoppE. rewrite H5.
    by rewrite -!RoppE Ropp_involutive.
  } rewrite -H6.
  destruct (Rlt_dec (m t_eps) (A_m + eps_0)).
  - by simpl.
  - simpl. 
    assert ((m t_eps < A_m + eps_0)%Re).
...
Lemma x_val_at_neighbor (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal):
  forall k:D, k \in (in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N) ->
  (x (t_eps+j)%N k <= A_M - (eps_j j eps_0 eps a))%Re.
Proof.
intros. rewrite in_setD in H.
assert ((k \notin X_M_t_e_i (eps_j j eps_0 eps a) A_M
                (t_eps + j)) /\ (k \in in_neighbor i)).
{ by apply /andP. } destruct H0. rewrite inE in H0.
destruct (Rgt_dec (x (t_eps + j) k)
...
Lemma x_left_ineq_1 (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal):
   i \in Normal -> (0 < a)%Re -> (a < 1)%Re ->
  (x (t_eps + j) i <= A_M - eps_j j eps_0 eps a)%Re ->
  let incl :=  incl_neigh_minus_extremes i x
            (t_eps + j) in
...
Proof.
intros. rewrite (@big_cond_simpl_lt i incl (fun i0:nat => (x (t_eps + j) (nth i incl i0) *
                    w (t_eps + j) (i, nth i incl i0))%Re)).
...
+ apply Rle_trans with a.
  ...
  - apply H3. apply mem_nth.
    ...
    ++ assert ((0< size incl)%N). { apply size_incl_not_0. }
       ...
       - by [].
       - apply H6.
+ apply Rle_trans with (x (t_eps + j) i).
  - assert (sorted_Dseq (incl_neigh_minus_extremes i x (t_eps + j)%N)).
...
Lemma R_i_gt_val_rel (i:D) (t:nat) :
  let incl :=  incl_neigh_minus_extremes i x t in
  forall j k:D, 
    k \in incl -> 
...
destruct H1.
destruct (Rgt_dec (x t j) (x t i)).
+ simpl in H0.
  rewrite /incl / incl_neigh_minus_extremes /remove_extremes in H.
  rewrite mem_filter in H. apply and_conv in H. destruct H.
  destruct H3.
  assert (Rle_dec (x t k) (x t i)
             \/ (index k (inclusive_neighbor_list i) <=
                 size (inclusive_neighbor_list i) - F - 1)%N).
  { by apply /orP. } destruct H5.
  - destruct (Rle_dec (x t k) (x t i)).
    * simpl in H5. apply Rle_trans with (x t i).
      ++ apply r0.
      ++ by apply Rlt_le.
    * by simpl in H5.
  - assert (sorted_Dseq (inclusive_neighbor_list i)).
    { apply lemma_1.inclusive_neighbors_sorted. } rewrite /sorted_Dseq in H6.
    apply H6.
    - apply H4.
    - apply H2.
...
Lemma exists_an_elem_in_out_set (i:D) (t:nat):
  ...
  nth i incl i0 \in Adversary ->
  exists k:D, k \in (Normal :&: (R_i_greater_than i t)).
Proof.
intros.
assert (exists (j:D), (j \in Normal) /\ (j \in R_i_greater_than i t)).
...
Lemma not_in_R_i_greater_than (i j:D) (t:nat):
  let incl :=  incl_neigh_minus_extremes i x t in
    (j \in incl \/ j \in R_i_less_than i t) ->
...
+ rewrite inE. rewrite /incl /incl_neigh_minus_extremes /remove_extremes in H.
  ...
  apply /nandP. left.
  apply /nandP. 
  assert (Rle_dec (x t j) (x t i)
             \/ (index j (inclusive_neighbor_list i) <=
                 size (inclusive_neighbor_list i) - F - 1)%N).
  { by apply /orP. } destruct H2.
  - left. destruct (Rle_dec (x t j) (x t i)).
    * simpl in H2. 
      destruct (Rgt_dec (x t j) (x t i)).
...
rewrite in_setC. 
apply (@not_in_R_i_greater_than i (nth i incl i0) t).
left. rewrite -/incl. 
apply mem_nth.
apply ltn_leq_trans with (size incl - 1).+1.
- by apply ltn_ord.
...
Lemma x_right_ineq_1 (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal):
 ...
 (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
    ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
       w (t_eps + j) (i, nth i incl i0)) * M (t_eps + j))%Re.
Proof.
intros. rewrite (@big_cond_simpl_gt i incl (fun i0:nat => (x (t_eps + j) (nth i incl i0) *
                    w (t_eps + j) (i, nth i incl i0))%Re)).
...
+ apply Rle_trans with a.
  ...
  - apply H3. apply mem_nth.
    ...
    ++ assert ((0< size incl)%N). { apply size_incl_not_0. }
       ...
       - by [].
       - apply H6.
+ assert ((nth i incl i0) \in Normal \/ (nth i incl i0) \in Adversary).
  { apply normal_or_adversary. }
  destruct H6.
  - by apply x_bound.
  - assert ((#|R_i_greater_than i (t_eps + j)%N| <= F)%N).
    { apply R_i_gt_bounded. } rewrite leq_eqVlt in H7.
    assert ( (#|R_i_greater_than i (t_eps + j)| == F)
              \/ (#|R_i_greater_than i (t_eps + j)| < F)%N).
    { by apply /orP. } destruct H8.
    * assert(R_i_greater_than i (t_eps + j) == Adversary \/
                  R_i_greater_than i (t_eps + j) != Adversary).
      { apply /orP.
        by destruct (R_i_greater_than i (t_eps + j) == Adversary).
      } destruct H9.
      ++ assert (R_i_greater_than i (t_eps + j) == Adversary ->
                  nth i incl i0 \notin Adversary).
         { by apply incl_does_not_contain_adversary. } specialize (H10 H9). 
         assert (~ (nth i incl i0 \in Adversary)).
         { by apply /negP. } contradiction.
      ++ assert (exists k:D, k \in (Normal :&: (R_i_greater_than  i (t_eps+j)%N))).
        { by apply exists_an_elem_in_out_set with i0.  } destruct H10 as [k H10]. rewrite in_setI in H10.
        assert ((k \in Normal) /\
                     (k \in R_i_greater_than i (t_eps + j))).
        { by apply /andP. } destruct H11.
        apply Rle_trans with (x (t_eps + j) k).
    ...
    * (** The last node has the value same as x_i(t) and since i is Normal,
        x_i <= M **)
      apply Rle_trans with (x (t_eps + j)%N (last (head i incl) (behead incl))).
      - apply last_incl_is_max. rewrite -/incl. apply mem_nth.
        ...
        + assert ((0< size incl)%N). { apply size_incl_not_0. }
          ...
          - by [].
          - apply H9.
      - assert (((x (t_eps + j) (last (head i incl ) (behead incl ))) = (x (t_eps + j) i))%Re).
...
Lemma x_val_decreases_1 (i:D) (a A_m A_M:R) (t_eps j N:nat) (eps eps_0: posreal):
  ...
  sum_f_R0
   ...
   (forall t : nat,
     (t >= t_eps)%coq_nat ->
     (m t > A_m - eps)%Re /\ (M t < A_M + eps)%Re) ->
  (x (t_eps + j) i <= A_M - eps_j j eps_0 eps a)%Re ->
  (x (t_eps + j.+1) i <=  A_M - eps_j j.+1 eps_0 eps a)%Re.
Proof.
intros. simpl. 
     ...
     - rewrite -RplusE.
       ...
       * apply Rle_trans with 
         ...
         ++ assert (( A_M - (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
            ...
            assert ((M (t_eps+j)%N < A_M + eps)%Re). 
            { apply H11. apply /leP. apply leq_addr. }
            remember  (w (t_eps + j) (i, i)) as w_ii.
            apply Rle_trans with ((1 - a) * M (t_eps + j)%N +
                                      a * (A_M - eps_j j eps_0 eps a))%Re.
            - assert (((A_M - eps_j j eps_0 eps a) * w_ii +
                        (1 - w_ii) * M (t_eps + j))%Re = 
                        (M (t_eps + j) - w_ii * ( M (t_eps + j) - (A_M - eps_j j eps_0 eps a)))%Re).
              { nra. } rewrite H20.
                assert (((1 - a) * M (t_eps + j) +
                            a * (A_M - eps_j j eps_0 eps a))%Re = 
                         (M (t_eps + j) - a * (M (t_eps + j) - (A_M - eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H21.
                apply Rplus_le_compat_l, Ropp_ge_le_contravar.
                apply Rmult_ge_compat_r.
...
++ rewrite (bigD1 (inord (index i incl))) //=. 
  ...
  + rewrite nth_index. 
    - rewrite -RplusE. 
      apply Rle_trans with ((M (t_eps +j)%N) * w (t_eps + j) (i, i) +
           \sum_(i0 < (size incl - 1).+1 | 
      ...
      * rewrite -RplusE -RmultE.
        ...
        rewrite (@big_nat_split i incl  
                  (fun i0 :nat => (x (t_eps + j) (nth i incl i0) *
                      w (t_eps + j) (i, nth i incl i0))%Re)).
        apply Rle_trans with
        (M (t_eps + j) * w (t_eps + j) (i, i) +  
          (((\sum_(i0 < (size incl - 1).+1 | 
                (i0 != (@inord  (size incl - 1) (index i incl))) &&
                (i0 < (@inord (size incl - 1) (index i incl)))%N)
        ...
        ++ apply Rle_trans with
            ...
            * assert ((M (t_eps + j) * w (t_eps + j) (i, i) +
                       ((\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j) (i, nth i incl i0)) *
                        (A_M - eps_j j eps_0 eps a) +
                       ...
                       ((w (t_eps + j) (i, i) + (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j) (i, nth i incl i0))) * M (t_eps+j) + 
                        (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j) (i, nth i incl i0)) *
                        (A_M - eps_j j eps_0 eps a))%Re).
              ...
              apply Rle_trans with ((1 - a) * M (t_eps + j)%N +
                                      a * (A_M - eps_j j eps_0 eps a))%Re.
              - assert (((1 - w_il) * M (t_eps + j) +
                              w_il * (A_M - eps_j j eps_0 eps a))%Re = 
                        (M (t_eps + j) - w_il * ( M (t_eps + j) - (A_M - eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H20.
                assert (((1 - a) * M (t_eps + j) +
                            a * (A_M - eps_j j eps_0 eps a))%Re = 
                         (M (t_eps + j) - a * (M (t_eps + j) - (A_M - eps_j j eps_0 eps a)))%Re).
...
Lemma x_in_incl_not_X_M (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal): forall (k:D),
  (k \in (in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N)) ->
    (x (t_eps+j)%N k <= (A_M - eps_j j eps_0 eps a))%Re. 
Proof.
...
Lemma in_neighbor_subset_incl (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal):
  ...
  {subset (in_neighbor i  :\: 
                X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)) <= [set x | x \in incl] :|: R_i_less_than i (t_eps+j)%N}.
Proof.
...
rewrite inE. 
destruct (Rgt_dec (x (t_eps + j) k) (A_M - eps_j j eps_0 eps a)).
+ by simpl in H1.
+ simpl in H1. 
  ...
  - left.
    ...
    * apply /andP. split.
      ++ apply /orP. by right.
      ++ apply /orP. left.
         destruct (Rle_dec (x (t_eps + j) k) (x (t_eps + j) i)).
         - by simpl.
         - simpl.
           assert ((x (t_eps + j) k <= x (t_eps + j) i)%Re).
  ...
  - rewrite leq_eqVlt in H3.
    ...
    * right. rewrite inE. apply /andP. split.
      ++ apply /andP. split.
         - destruct (Rlt_dec (x (t_eps + j) k) (x (t_eps + j) i)).
           * by simpl.
           * simpl.
             assert ((x (t_eps + j) k < x (t_eps + j) i)%Re).
...
Lemma s_subset_incl (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal) (s: seq D): 
  ...
  {subset s <= in_neighbor i  :\: 
                X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)} ->
  {subset s <= [set x | x \in incl] :|: R_i_less_than i (t_eps+j)%N}.
Proof.
intros.
assert ({subset [set x | x \in s] <= [set x | x \in incl] :|: R_i_less_than i (t_eps+j)%N}).
{ apply /subsetP.
  apply (@subset_trans D [set x | x \in s] 
   (in_neighbor i  :\: 
                  X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j))
    ([set x | x \in incl] :|: R_i_less_than i (t_eps+j)%N)). 
...
Lemma k_in_incl_bounded (i:D) (a A_m A_M:R) (t_eps j N:nat) (eps eps_0: posreal):
  ...
  sum_f_R0
   ...
   (forall t : nat,
     (t >= t_eps)%coq_nat ->
     (m t > A_m - eps)%Re /\ (M t < A_M + eps)%Re) ->
  (F + 1 <= #|in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)|)%N ->
  (x (t_eps + j) i > A_M - eps_j j eps_0 eps a)%Re ->
  (forall k: 'I_(size incl - 1).+1,
               (x (t_eps + j) (nth i incl k) <= M (t_eps + j))%Re).
Proof.
intros.
...
+ rewrite leq_eqVlt in H13.
  ...
  - assert (k = inord (index i incl)).
    ...
    * rewrite subn1 prednK.
      ++ rewrite index_mem. apply i_in_incl.
      ++ apply size_incl_not_0.
  - apply Rle_trans with  (x (t_eps + j) (nth i incl (@inord (size incl -1) (index i incl))))%Re.
    * rewrite inordK.
      ++ rewrite nth_index.
         - assert (sorted_Dseq incl).
    ...
    * rewrite inordK.
      ++ rewrite nth_index.
         ...
         - apply i_in_incl.
      ++ rewrite subn1 prednK. rewrite index_mem. apply i_in_incl. apply size_incl_not_0.
+ assert ((nth i incl k) \in Normal \/ (nth i incl k) \in Adversary).
  { apply normal_or_adversary. }
  destruct H14.
  - by apply x_bound.
  - assert ((#|R_i_greater_than i (t_eps + j)%N| <= F)%N).
...
assert (exists s, [/\ uniq s, size s = (F+1)%N & 
              {subset s <= (in_neighbor i :\: X_M_t_e_i  (eps_j j eps_0 eps a) A_M (t_eps+j)%N)}]).
{ by apply /card_geqP. } destruct H16 as [s H16].
destruct H16.
assert (exists k:D, k \in [set x | x \in s] :&: [set x | x \in incl]).
{ apply exists_in_intersection with (R_i_less_than i (t_eps+j)%N) F.
   ...
   * apply Rle_trans with 
     ...
     ++ assert (( A_M - (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
        ...
        - assert ((w_ik * (A_M - eps_j j eps_0 eps a) +
                    (1 - w_ik) * M (t_eps + j))%Re = 
                        (M (t_eps + j) - w_ik * ( M (t_eps + j) - (A_M - eps_j j eps_0 eps a)))%Re).
          { nra. } rewrite H25.
          assert (((1 - a) * M (t_eps + j) +
                            a * (A_M - eps_j j eps_0 eps a))%Re = 
                         (M (t_eps + j) - a * (M (t_eps + j) - (A_M - eps_j j eps_0 eps a)))%Re).
...
Lemma exists_an_elem_in_out_set_R_lt (i:D) (t:nat):
  ...
  forall i0 : 'I_(size incl - 1).+1, 
  nth i incl i0 \in Adversary ->
  exists k:D, k \in (Normal :&: (R_i_less_than i t)).
Proof.
intros.
assert (exists (j:D), (j \in Normal) /\ (j \in R_i_less_than i t)).
...
Lemma not_in_R_i_less_than (i j:D) (t:nat):
  let incl :=  incl_neigh_minus_extremes i x t in
    (j \in incl \/ j \in R_i_greater_than i t) ->
...
+ rewrite inE. rewrite /incl /incl_neigh_minus_extremes /remove_extremes in H.
  ...
  apply /nandP. left.
  apply /nandP. 
  assert (Rge_dec (x t j) (x t i)
             \/ (F <= index j (inclusive_neighbor_list i))%N).
  { by apply /orP. } destruct H2.
  - left. destruct (Rge_dec (x t j) (x t i)).
    * simpl in H2. 
      destruct (Rlt_dec (x t j) (x t i)).
...
+ simpl in H0.
  ...
  rewrite mem_filter in H. apply and_conv in H. destruct H.
  destruct H3.
  assert (Rge_dec (x t k) (x t i)
             \/ (F <= index k (inclusive_neighbor_list i))%N).
  { by apply /orP. } destruct H5.
  - destruct (Rge_dec (x t k) (x t i)).
    * simpl in H5. apply Rle_trans with (x t i).
      ++ by apply Rlt_le.
      ++ apply Rge_le, r0.
    * by simpl in H5.
...
+ assert ((nth i incl i0) \in Normal \/ (nth i incl i0) \in Adversary).
  ...
  destruct H6.
  - by apply x_bound.
  - assert ((#|R_i_less_than i (t_eps + j)%N| <= F)%N).
    { apply R_i_lt_bounded. } rewrite leq_eqVlt in H7.
    assert ( (#|R_i_less_than i (t_eps + j)| == F)
              \/ (#|R_i_less_than i (t_eps + j)| < F)%N).
    { by apply /orP. } destruct H8.
    * assert(R_i_less_than i (t_eps + j) == Adversary \/
                  R_i_less_than i (t_eps + j) != Adversary).
      { apply /orP.
        by destruct (R_i_less_than i (t_eps + j) == Adversary).
      } destruct H9.
      ++ assert (R_i_less_than i (t_eps + j) == Adversary ->
                  nth i incl i0 \notin Adversary).
    ...
    * (** The last node has the value same as x_i(t) and since i is Normal,
        x_i <= M **)
      apply Rle_trans with (x (t_eps + j)%N (nth i (incl_neigh_minus_extremes i x (t_eps + j)) 0)).
      - assert (((x (t_eps+j) (nth i (incl_neigh_minus_extremes i x (t_eps + j)) 0)) = (x (t_eps + j) i))%Re).
       ...
       * apply Rge_trans with 
         ...
         ++ assert (( A_m + (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
            ...
            { assert ((\sum_(i0 < (size incl - 1).+1 | 
              ...
              assert ((m (t_eps+j)%N > A_m - eps)%Re). 
              { apply H11. apply /leP. apply leq_addr. }
              apply Rge_trans with ((1 - a) * m (t_eps + j)%N +
                                      a * (A_m + eps_j j eps_0 eps a))%Re.
              - assert (((A_m + eps_j j eps_0 eps a) * w_ii +
                          (1 - w_ii) * m (t_eps + j))%Re = 
                        (m (t_eps + j) - w_ii * ( m (t_eps + j) - (A_m + eps_j j eps_0 eps a)))%Re).
                ...
                assert ((- (w_ii * (m (t_eps + j) - (A_m + eps_j j eps_0 eps a))))%Re = 
                          (w_ii * ((A_m + eps_j j eps_0 eps a) - m (t_eps + j)))%Re). { nra. }
                rewrite H21. clear H21.
                assert ((-(a * (m (t_eps + j) - (A_m + eps_j j eps_0 eps a))))%Re = 
                           (a * ((A_m + eps_j j eps_0 eps a) - m (t_eps + j)))%Re). { nra. }
...
++ rewrite (bigD1 (inord (index i incl))) //=. 
  ...
  + rewrite nth_index. 
    - rewrite -RplusE. 
      apply Rge_trans with ((m (t_eps +j)%N) * w (t_eps + j) (i, i) +
           \sum_(i0 < (size incl - 1).+1 | 
           i0 != inord (index i incl))
              (x (t_eps + j) (nth i incl i0) *
               w (t_eps + j) (i, nth i incl i0))%Re).
      ...
      * rewrite -RplusE -RmultE.
        ...
        (m (t_eps + j) * w (t_eps + j) (i, i) +  
          (((\sum_(i0 < (size incl - 1).+1 | 
                (i0 != (@inord  (size incl - 1) (index i incl))) &&
                (i0 < (@inord (size incl - 1) (index i incl)))%N)
                    w (t_eps + j) (i, nth i incl i0)) * (m (t_eps+j)%N))%Re +
          (\sum_(i0 < (size incl - 1).+1 | (i0
                                    != 
                                   (@inord  (size incl - 1) (index i incl))) &&
                                    ~~
         ...
         ++ apply Rge_trans with
            ...
            * assert ((m (t_eps + j) * w (t_eps + j) (i, i) +
                       ((\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j) (i, nth i incl i0)) * m (t_eps+j)%N
                         +
                        (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j) (i, nth i incl i0)) * (A_m + eps_j j eps_0 eps a)))%Re = 
              ...
              apply Rge_trans with ((1 - a) * m (t_eps + j)%N +
                                      a * (A_m + eps_j j eps_0 eps a))%Re.
              - assert (((1 - w_ih) * m (t_eps + j) +
                              w_ih * (A_m + eps_j j eps_0 eps a))%Re = 
                        (m (t_eps + j) - w_ih * ( m (t_eps + j) - (A_m + eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H20.
                assert (((1 - a) * m (t_eps + j) +
                            a * (A_m + eps_j j eps_0 eps a))%Re = 
                         (m (t_eps + j) - a * (m (t_eps + j) - (A_m + eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H21.
                apply Rplus_ge_compat_l. 
                assert ((- (w_ih * (m (t_eps + j) - (A_m + eps_j j eps_0 eps a))))%Re = 
                          (w_ih * ((A_m + eps_j j eps_0 eps a) - m (t_eps + j)))%Re). { nra. }
...
Lemma in_neighbor_subset_incl_last (i:D) (a A_m:R) (t_eps j:nat) (eps eps_0: posreal):
  ...
  let incl :=  incl_neigh_minus_extremes i x
            (t_eps + j) in 
  {subset (in_neighbor i  :\: 
                X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)) <= [set x | x \in incl] :|: R_i_greater_than i (t_eps+j)%N}.
Proof.
intros. rewrite /sub_mem //=. move=> k.
intros. rewrite inE in H1. apply and_2_conv in H1.
destruct H1. rewrite inE in H1. rewrite in_setU. apply /orP.
rewrite inE. 
destruct (Rlt_dec (x (t_eps + j) k) (A_m + eps_j j eps_0 eps a)).
+ by simpl in H1.
+ simpl in H1. 
  ...
  - left.
    ...
    * apply /andP. split.
      ++ apply /orP. left.
         destruct (Rge_dec (x (t_eps + j) k) (x (t_eps + j) i)).
         - by simpl.
         - simpl.
           assert ((x (t_eps + j) k >= x (t_eps + j) i)%Re).
  ...
  - rewrite leq_eqVlt in H3.
    ...
    * right. rewrite inE. apply /andP. split.
      ++ apply /andP. split.
         - destruct (Rgt_dec (x (t_eps + j) k) (x (t_eps + j) i)).
           * by simpl.
           * simpl.
             assert ((x (t_eps + j) k > x (t_eps + j) i)%Re).
...
Lemma s_subset_incl_last (i:D) (a A_m:R) (t_eps j:nat) (eps eps_0: posreal) (s: seq D): 
  ...
  {subset s <= in_neighbor i  :\: 
                X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)} ->
  {subset s <= [set x | x \in incl] :|: R_i_greater_than i (t_eps+j)%N}.
Proof.
intros.
assert ({subset [set x | x \in s] <= [set x | x \in incl] :|: R_i_greater_than i (t_eps+j)%N}).
{ apply /subsetP.
  apply (@subset_trans D [set x | x \in s] 
   (in_neighbor i  :\: 
                  X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j))
    ([set x | x \in incl] :|: R_i_greater_than i (t_eps+j)%N)). 
  + apply /subsetP. rewrite /sub_mem //=. intros.
    rewrite inE in H2. rewrite /sub_mem in H1. 
    by specialize (H1 x H2).
...
destruct H16.
assert (exists k:D, k \in [set x | x \in s] :&: [set x | x \in incl]).
{ apply exists_in_intersection with (R_i_greater_than i (t_eps+j)%N) F.
  + assert (#|s| = size s). { by apply /card_uniqP. }
    by rewrite H19.
   ...
   * apply Rle_trans with 
     ...
     ++ assert (( A_m + (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
        ...
        - assert ((w_ik * (A_m + eps_j j eps_0 eps a) +
          ...
          { nra. } rewrite H26.
          apply Rplus_le_compat_l. 
          assert ((- (w_ik * (m (t_eps + j) - (A_m + eps_j j eps_0 eps a))))%Re = 
                          (w_ik * ((A_m + eps_j j eps_0 eps a) - m (t_eps + j)))%Re). { nra. }
...
Lemma X_M_size_decreases_from_0_1 
  ...
  (0 < F + 1 <= #|D|)%N ->
   is_lim_seq [eta M] A_M ->
   (0 < a)%Re -> (a < 1)%Re ->
   (eps < a ^ #|Normal| / (1 - a ^ #|Normal|) * eps_0)%Re ->
  (forall i:D, (*i \in Normal ->  *)
     (forall k : D,
          k \in incl_neigh_minus_extremes i x (t_eps + 0) ->
          (a <= w (t_eps + 0) (i, k))%Re) /\
     (sum_f_R0
        (fun n0 : nat =>
         w (t_eps + 0)
           (i,
           nth i
             (incl_neigh_minus_extremes i x (t_eps + 0))
             n0))
        (size (incl_neigh_minus_extremes i x (t_eps + 0)) -
         1) = 1%Re)) ->
  (forall t : nat,
    (t >= t_eps)%coq_nat ->
    (m t > A_m - eps)%Re /\ (M t < A_M + eps)%Re) -> 
                ...
                (F + 1 <=
                   #|in_neighbor i
                     :\: X_M_t_e_i (eps_j 0 eps_0 eps a) A_M
                           (t_eps + 0)|)%N ) -> 
  (#|X_M_t_e_i (eps_j 1 eps_0 eps a) A_M (t_eps + 1)
   :&: Normal| <
   #|X_M_t_e_i (eps_j 0 eps_0 eps a) A_M t_eps
     :&: Normal|)%N.
Proof.
intros.
...
+ apply /subsetP. rewrite /sub_mem //=. move => i H9.
  ...
  assert ( (i \in X_M_t_e_i (a * eps_0 - (1 - a) * eps)
             A_M (t_eps + 1)) /\ (i \in Normal)).
  { by apply /andP. } destruct H10 as [H10 H11].
  split.
  - rewrite inE. rewrite inE in H10.
    destruct (Rgt_dec (x (t_eps + 1) i)
                (A_M - (a * eps_0 - (1 - a) * eps))).
    * simpl in H10. 
      destruct (Rgt_dec (x t_eps i) (A_M - eps_0)).
      ++ by simpl.
      ++ simpl. apply Rnot_gt_le in n.
         assert (( x (t_eps + 1) i <= A_M - (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_decreases_1 i a A_m A_M t_eps 0%N #|Normal| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
+ apply /subsetPn. rewrite /eps_j. destruct H8 as [i H8].
  ...
  - destruct H8 as [H8 H9].
    ...
    * simpl in H8. 
      ...
      ++ simpl.
         assert ((x (t_eps + 1) i <= A_M - (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_decreases_2 i a A_m A_M t_eps 0%N #|Normal| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
+ apply /subsetP. rewrite /sub_mem //=. move => i H9.
  ...
  split.
  - rewrite inE. rewrite inE in H10.
    destruct (Rlt_dec (x (t_eps + 1) i)
                (A_m + (a * eps_0 - (1 - a) * eps))).
    * simpl in H10. 
      destruct (Rlt_dec (x t_eps i) (A_m + eps_0)).
      ++ by simpl.
      ++ simpl. apply Rnot_lt_ge in n.
         assert (( x (t_eps + 1) i >= A_m + (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_increases_1 i a A_m A_M t_eps 0%N #|Normal| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
+ apply /subsetPn. rewrite /eps_j. destruct H8 as [i H8].
  ...
  - destruct H8 as [H8 H9].
    ...
    * simpl in H8. 
      ...
      ++ simpl.
         assert ((x (t_eps + 1) i >= A_m + (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_increases_2 i a A_m A_M t_eps 0%N #|Normal| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
Lemma adversary_not_normal_lem:
  Adversary = ~: Normal.
Proof.
rewrite -setTD. apply eq_finset. unfold eqfun. move =>i.
rewrite !inE.
by destruct (corrupt i == true).
Qed.

Lemma adversary_max: forall (A B: {set D}),
  F_totally_bounded ->
  A :&: B = set0 ->
  (#|A :\: Normal| + #|B :\: Normal| <=F)%N.
Proof.
intros. rewrite -cardsUI. rewrite -setDIl.
rewrite H0 set0D cards0 addn0 -setDUl.
apply leq_trans with #|Adversary|.
...
Lemma normal_node_cond (t_eps: nat) (A_M A_m:R) (eps_0 eps:posreal):
  ...
  (A_M - eps_0 > A_m + eps_0)%Re ->
  (F + 1 <=
     #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps)
         (F + 1)| +
     #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps)
         (F + 1)|)%N ->
  exists i:D, (i \in X_M_t_e_i eps_0 A_M t_eps /\ i \in Normal /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_M_t_e_i eps_0 A_M t_eps|)%N) \/
               (i \in X_m_t_e_i eps_0 A_m t_eps /\ i \in Normal /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_m_t_e_i eps_0 A_m t_eps|)%N).
Proof.
intros.
assert ( #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)|  = 
           (#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1) :&: Normal | +
             #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1) :\: Normal|)%N).
{ by rewrite cardsID. }
assert ( #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)| = 
        (#|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1) :&: Normal | +
             #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1) :\: Normal|)%N).
...
assert ((#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)
           :&: Normal| +
         #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)
           :\: Normal| +
         (#|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
            :&: Normal| +
          #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
            :\: Normal|))%N = 
      ((#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)
           :&: Normal| + #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
            :&: Normal|) + 
      ( #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)
           :\: Normal| + #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
            :\: Normal|))%N).
{ rewrite !addnA. by rewrite sum_eq. } rewrite H4 in H1. clear H4.
apply (@add_max_bound (#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)
       :&: Normal| +
     #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
       :&: Normal|)%N   (#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)
        :\: Normal| +
      #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
        :\: Normal|)%N F) in H1.
+ assert (exists i:D, 
          i \in ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)
       :&: Normal) :|:
      (Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
       :&: Normal))).
  { apply /card_gt0P. rewrite cardsU.
    assert ((#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1) :&: Normal
               :&: (Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)
                    :&: Normal)|)%N = 0%N).
    { rewrite setIACA. rewrite setIid.
      ...
      rewrite inE.
      assert ((i \in Xi_S_r (X_m_t_e_i eps_0 A_m t_eps) (F + 1)) = 
                ((i \in X_m_t_e_i eps_0 A_m t_eps) && 
                  (F + 1 <=
                    #|in_neighbor i :\: X_m_t_e_i eps_0 A_m t_eps|)%N)).
      { by rewrite inE. } rewrite H4.
      rewrite and_distr. apply /nandP. left.
      apply /nandP.
      assert ([disjoint (X_M_t_e_i eps_0 A_M t_eps) & (X_m_t_e_i eps_0 A_m t_eps)]).
      { by apply X_M_X_m_disjoint. }
      apply (@implies_simp (i \in (X_M_t_e_i eps_0 A_M t_eps))
                (i \notin X_m_t_e_i eps_0 A_m t_eps)).
      intros.
      assert ((i \in (X_m_t_e_i eps_0 A_m t_eps) = false) ->
             i \notin (X_m_t_e_i eps_0 A_m t_eps)). 
      { intros. by apply ssrbool.negbT. }
      apply H7. 
      by apply (@disjointFr D (X_M_t_e_i eps_0 A_M t_eps) (X_m_t_e_i eps_0 A_m t_eps) i). 
    } by rewrite H4 subn0.
  } destruct H4 as [ i H4].
  exists i.
  assert ( i \in (Xi_S_r (X_M_t_e_i eps_0 A_M t_eps)
                    (F + 1) :&: Normal) \/ 
           i \in (Xi_S_r (X_m_t_e_i eps_0 A_m t_eps)
                    (F + 1) :&: Normal)).
  ...
  - left.
    ...
    rewrite inE in H6.
    assert ((i \in X_M_t_e_i eps_0 A_M t_eps) /\
               (F + 1 <=
                #|in_neighbor i :\: X_M_t_e_i eps_0 A_M t_eps|)%N).
...
+ apply adversary_max.
  ...
  - apply eq_finset. unfold eqfun. move=>i.
    ...
    apply (@disjointFr D (X_M_t_e_i eps_0 A_M t_eps) (X_m_t_e_i eps_0 A_m t_eps) i).
    * by apply X_M_X_m_disjoint.
    * by []. 
Qed.
...
assert ( exists i:D, i \in (X_M_t_e_i eps_0 A_M t_eps) /\ i \in Normal).
...
Proof.
intros.
assert ( exists i:D, i \in (X_M_t_e_i eps_0 A_M t_eps) /\
                        i \in Normal).
...
+ split.
  ...
  - rewrite eqn_leq in H3. apply split_ineq in H3.
    ...
    assert ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)) \subset 
              (X_M_t_e_i eps_0 A_M t_eps)).
    ...
    assert ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1) \subset
               X_M_t_e_i eps_0 A_M t_eps) && 
            ((#|X_M_t_e_i eps_0 A_M t_eps| <=
                #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1)|)%N)).
    { apply /andP. by split. }
    assert (Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1) == 
                  X_M_t_e_i eps_0 A_M t_eps).
    { by rewrite eqEcard. }
    ...
    assert ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1) \subset
                  X_M_t_e_i eps_0 A_M t_eps) /\
                 (X_M_t_e_i eps_0 A_M t_eps \subset
                  Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1))).
    { by apply /andP. } destruct H10 as [H10 H11].
    rewrite subsets_disjoint in H11.
    apply (@disjointFr D (X_M_t_e_i eps_0 A_M t_eps)
              (~: (Xi_S_r (X_M_t_e_i eps_0 A_M t_eps) (F + 1))) i) in H11.
...
Lemma exists_at_jp1_implies_at_j (t_eps j N: nat) (a A_m A_M :R) (eps eps_0: posreal):
  ...
  (forall i:D, (*i \in Normal ->  *)
    let incl :=
         incl_neigh_minus_extremes i x (t_eps + j)
         in
       (forall k : D,
        k \in incl ->
        (a <= w (t_eps + j) (i, k))%Re) /\
...
assert ((i \in X_M_t_e_i (eps_j j.+1 eps_0 eps a) A_M
              (t_eps + j.+1)) /\ (i \in Normal)).
{ by apply /andP. } destruct H13.
apply /andP. split.
+ rewrite inE. rewrite inE in H13.
  destruct (Rgt_dec (x (t_eps + j.+1) i)
                (A_M - eps_j j.+1 eps_0 eps a)).
  - simpl in H13.
    destruct (Rgt_dec (x (t_eps + j) i)
                  (A_M - eps_j j eps_0 eps a)).
    * by simpl.
    * simpl. apply Rnot_gt_le in n.
      assert ((x (t_eps + j.+1) i <=
...
apply /andP. split.
+ rewrite inE. rewrite inE in H13.
  destruct (Rlt_dec (x (t_eps + j.+1) i)
                (A_m + eps_j j.+1 eps_0 eps a)).
  - simpl in H13.
    destruct (Rlt_dec (x (t_eps + j) i)
                  (A_m + eps_j j eps_0 eps a)).
    * by simpl.
    * simpl. apply Rnot_lt_ge in n.
      assert ((x (t_eps + j.+1) i >=
...
Lemma max_bound_var (A_M:R) (t:nat):
  F_totally_bounded ->
  (0 < F + 1 <= #|D|)%N ->
  (forall (i:D), 
    i \in Normal -> (x t i < A_M)%Re) ->
...
Lemma X_M_card_le (t_eps j N: nat) (a A_M A_m:R) (eps_0 eps:posreal):
  ...
  (#|X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)
   :&: Normal| <=
 #|X_M_t_e_i (eps_j j.-1 eps_0 eps a) A_M (t_eps + j.-1)
   :&: Normal|)%N.
Proof.
intros.
...
assert ((k \in X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)) /\
          (k \in Normal)).
...
+ rewrite inE in H12. rewrite inE.
  destruct (Rgt_dec (x (t_eps + j) k) (A_M - eps_j j eps_0 eps a)).
  - simpl.
    destruct (Rgt_dec (x (t_eps + j.-1) k) (A_M - eps_j j.-1 eps_0 eps a)).
    * by simpl.
    * simpl. apply Rnot_gt_le in n.
      assert ((x (t_eps + j) k <= A_M - eps_j j eps_0 eps a)%Re).
      { assert (j = (j-1).+1). { rewrite subn1. by rewrite prednK. }
        rewrite H14.
...
+ rewrite inE in H12. rewrite inE.
  destruct (Rlt_dec (x (t_eps + j) k) (A_m + eps_j j eps_0 eps a)).
  - simpl.
    destruct (Rlt_dec (x (t_eps + j.-1) k) (A_m + eps_j j.-1 eps_0 eps a)).
    * by simpl.
    * simpl. apply Rnot_gt_le in n.
      assert ((x (t_eps + j) k >= A_m + eps_j j eps_0 eps a)%Re).
      { assert (j = (j-1).+1). { rewrite subn1. by rewrite prednK. }
  ...
  (exists i:D, (i \in X_M_t_e_i eps_0 A_M t_eps /\ i \in Normal /\
               ...
               (i \in X_m_t_e_i eps_0 A_m t_eps /\ i \in Normal /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_m_t_e_i eps_0 A_m t_eps|)%N)) ->
  (#|X_M_t_e_i (eps_j 0 eps_0 eps a) A_M (t_eps + 0)
   :&: Normal| +
 #|X_m_t_e_i (eps_j 0 eps_0 eps a) A_m (t_eps + 0)
   :&: Normal| <= N)%N ->
  (#|X_M_t_e_i (eps_j 1 eps_0 eps a) A_M (t_eps + 1)
...
+ destruct H10. destruct H12.
  ...
  - rewrite ltn_add2r. apply card_decreases. exists i.
    ...
    * rewrite inE. apply /nandP. left. rewrite inE.
      ...
      destruct (Rgt_dec (x t_eps i) (A_M - eps_0)).
      ++ simpl in H11.
         destruct (Rgt_dec (x (t_eps + 1) i) (A_M - eps_j 1 eps_0 eps a)).
         + simpl.
           assert ((x (t_eps + 1) i <=  A_M - eps_j 1 eps_0 eps a)%Re).
    ...
    * apply /subsetP. rewrite /sub_mem //=. move=> p H14.
         ...
         * rewrite inE. rewrite inE in H15.
           rewrite addn0.
           destruct (Rgt_dec (x (t_eps + 1) p)
                          (A_M - (a * eps_j 0 eps_0 eps a  - (1 - a) * eps))).
           ++ simpl in H15.
              destruct (Rgt_dec (x (t_eps) p)
                                (A_M - eps_0 )).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x (t_eps + 1) p <= A_M -
...
+ destruct H10. destruct H12.
  ...
  - rewrite ltn_add2l. apply card_decreases. exists i.
    ...
    * rewrite inE. apply /nandP. left. rewrite inE.
      ...
      destruct (Rlt_dec (x t_eps i) (A_m +eps_0)).
      ++ simpl in H10.
         destruct (Rlt_dec (x (t_eps + 1) i) (A_m + eps_j 1 eps_0 eps a)).
         + simpl. 
           assert ((x (t_eps + 1) i >=  A_m + eps_j 1 eps_0 eps a)%Re).
    ...
    * apply /subsetP. rewrite /sub_mem //=. move=> p H14.
         ...
         * rewrite inE. rewrite inE in H15.
           rewrite addn0.
           destruct (Rlt_dec (x (t_eps + 1) p)
                          (A_m + (a * eps_j 0 eps_0 eps a  - (1 - a) * eps))).
           ++ simpl in H15.
              destruct (Rlt_dec (x (t_eps) p)
                                (A_m + eps_0 )).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x (t_eps + 1) p >= A_m +
...
intros.
rewrite -cardsUI.
assert (#|X_M_t_e_i (eps_j 0 eps_0 eps a) A_M (t_eps + 0)
         :&: Normal
         :&: (X_m_t_e_i (eps_j 0 eps_0 eps a) A_m (t_eps + 0)
              :&: Normal)| = 0%N).
{ rewrite -setIACA setIid.
  assert (X_M_t_e_i (eps_j 0 eps_0 eps a) A_M (t_eps + 0)
            :&: X_m_t_e_i (eps_j 0 eps_0 eps a) A_m (t_eps + 0) = set0).
  ...
  (forall t : nat,
   ...
   (forall (i:D)(t:nat), (*i \in Normal -> *) 
    ...
    (exists i : D,
      i \in X_M_t_e_i eps_0 A_M t_eps /\
      i \in Normal /\
      (F + 1 <= #|in_neighbor i :\: X_M_t_e_i eps_0 A_M t_eps|)%N \/
      i \in X_m_t_e_i eps_0 A_m t_eps /\
      i \in Normal /\
      (F + 1 <= #|in_neighbor i :\: X_m_t_e_i eps_0 A_m t_eps|)%N) ->
     (forall j:nat, (0<j)%N -> (j<=N)%N ->
                (eps_j j eps_0 eps a < eps_j (j-1)%N eps_0 eps a)%Re ->   
                 ( ( (0 < #|(X_M_t_e_i (eps_j j eps_0 eps a ) A_M (t_eps+j)%N) :&: Normal|)%N ->
                      (#|(X_M_t_e_i (eps_j j eps_0 eps a ) A_M (t_eps+j)%N) :&: Normal| <
                  #| (X_M_t_e_i (eps_j (j-1)%N eps_0 eps a)%N A_M (t_eps+(j-1))%N) :&: Normal|)%N) \/
                  ( (0 < #|(X_m_t_e_i (eps_j j eps_0 eps a ) A_m (t_eps+j)%N) :&: Normal|)%N ->
                    (#|(X_m_t_e_i (eps_j j eps_0 eps a ) A_m (t_eps+j)%N) :&: Normal| <
                      #| (X_m_t_e_i (eps_j (j-1)%N eps_0 eps a)%N A_m (t_eps+(j-1))%N) :&: Normal|)%N))) ->
    (exists t : nat, (M t < A_M)%Re \/ (m t > A_m)%Re).
Proof.
intros.
assert (exists T:nat,(T<=N)%N /\ 
          (#|(X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T)%N) :&: Normal| = 0%N \/
           #|(X_m_t_e_i (eps_j T eps_0 eps a) A_m (t_eps + T)%N) :&: Normal| = 0%N)).
{ apply sj_ind_var.
...
+ left. apply cards0_eq in H14.
  ...
  assert ((forall i : D, i \in Normal -> (x (t_eps+T)%N i < A_M)%Re) -> 
              (M (t_eps + T) < A_M)%Re).
  ...
  assert ([disjoint
          X_M_t_e_i (eps_j T eps_0 eps a) A_M
            (t_eps + T) & Normal] ->
           i \in Normal ->
           i \notin (X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T))).
  { intros. 
    assert ((i \in (X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T)) = false) ->
               i \notin (X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T))). 
    { intros. by apply ssrbool.negbT. } 
    apply H20. move: H18 H19. apply disjointFl.
  } specialize (H18 H15 H17).
  rewrite inE in H18.
  destruct (Rgt_dec (x (t_eps + T) i)
...
Proof.
intros. 
assert (M (t_eps+j) \in map (fun i:D => x (t_eps +j) i) (enum Normal)).
{ rewrite /M. 
  apply (@bigmaxr_mem _ 0%Re (map (fun i:D => x (t_eps+j) i) (enum Normal))).
  rewrite size_map. destruct H. by apply size_normal_gt_0.
} 
assert (exists2 i:D, i \in (enum Normal) &
            (M (t_eps+j) = (fun i:D => x (t_eps+j) i) i)).
{ by apply /mapP. }
destruct H8 as [i H8]. exists i.
split.
+ rewrite inE. rewrite -H9.
  destruct (Rgt_dec (M (t_eps+j)) (A_M - eps_j j eps_0 eps a)).
  - by simpl.
  - simpl.  
    assert ((A_M - eps_j j eps_0 eps a < M (t_eps+j))%Re).
    { apply Rlt_le_trans with (A_M - 0)%Re.
      + apply Rplus_lt_compat_l. apply Ropp_gt_lt_contravar.
...
split.
+ rewrite inE.
  assert (m (t_eps +j)= x (t_eps +j) i). 
  { rewrite -[LHS]Ropp_involutive. rewrite !RoppE. rewrite H9.
    by rewrite -!RoppE Ropp_involutive.
  } rewrite -H10.
  destruct (Rlt_dec (m (t_eps+j)) (A_m + eps_j j eps_0 eps a)).
  - by simpl.
  - simpl. 
    assert ((m (t_eps +j)  < A_m + eps_j j eps_0 eps a)%Re).
    { apply Rle_lt_trans with (A_m + 0)%Re.
      + assert ((A_m + 0)%Re = A_m). { nra. }
...
Lemma X_M_X_m_disjoint_at_j:
  ...
  [disjoint (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) & 
    (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j))].
Proof.
intros. rewrite -setI_eq0. apply /eqP.
apply eq_finset. unfold eqfun. move=>i.
...
rewrite !in_set. apply /andP. 
apply /andP. apply /nandP.
destruct (Rgt_dec (x (t_eps+j) i) (A_M - (eps_j j eps_0 eps a))).
+ simpl. 
  destruct (Rlt_dec (x (t_eps+j) i) (A_m + (eps_j j eps_0 eps a))).
  - simpl. left.
    assert ((x (t_eps+j) i > A_m + (eps_j j eps_0 eps a))%Re).
    { apply Rgt_trans with (A_M - (eps_j j eps_0 eps a))%Re; by []. }
    assert ((x (t_eps+j) i > A_m + (eps_j j eps_0 eps a))%Re -> 
            ~(x (t_eps+j) i < A_m + (eps_j j eps_0 eps a))%Re). { nra. }
    specialize (H1 H0).
    contradiction.
  - simpl. by right.
+ simpl. by left.
...
- rewrite cardsCs.
  ...
  rewrite ltn_subrL. apply /andP. split.
  * rewrite card_gt0. apply /set0Pn.
    exists (nth key (enum (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j))) 0%N).
    rewrite inE -disjoints1.
    apply single_element_disjoint.
    ++ by [].
    ++ rewrite disjoint_sym.
...
Lemma X_m_mem_j:
  forall (t_eps j:nat) (a A_M A_m :R) (eps_0 eps:posreal),
  (0 < #|D|)%N ->  (A_M - (eps_j j eps_0 eps a) > A_m + (eps_j j eps_0 eps a))%Re -> 
  (0 < #|X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)|)%N -> 
  X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j) \proper Vertex.
Proof.
intros.
rewrite properEcard.
apply /andP. split.
...
- rewrite cardsCs.
  ...
  * rewrite card_gt0. apply /set0Pn.
    ...
    apply single_element_disjoint.
    ++ by [].
    ++ by apply (@X_M_X_m_disjoint_at_j t_eps j a A_M A_m eps_0 eps).
  * apply H.
Qed.
...
Lemma normal_node_cond_at_j (t_eps j N: nat) (a A_M A_m:R) (eps_0 eps:posreal):
  ...
  (eps < a ^ N / (1 - a ^ N) * eps_0)%Re ->
  (F + 1 <=
     #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j))
         (F + 1)| +
     #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j))
         (F + 1)|)%N ->
  exists i:D, (i \in X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j) /\ i \in Normal /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)|)%N) \/
               (i \in X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j) /\ i \in Normal /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)|)%N).
Proof.
intros.
assert ( #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)|  = 
           (#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1) :&: Normal | +
             #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1) :\: Normal|)%N).
{ by rewrite cardsID. }
assert ( #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)| = 
        (#|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1) :&: Normal | +
             #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1) :\: Normal|)%N).
...
assert ((#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)
           :&: Normal| +
         #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)
           :\: Normal| +
         (#|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
            :&: Normal| +
          #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
            :\: Normal|))%N = 
      ((#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)
           :&: Normal| + #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
            :&: Normal|) + 
      ( #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)
           :\: Normal| + #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
            :\: Normal|))%N).
{ rewrite !addnA. by rewrite sum_eq. } rewrite H9 in H6. clear H9.
apply (@add_max_bound (#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)
       :&: Normal| +
     #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
       :&: Normal|)%N   (#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)
        :\: Normal| +
      #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
        :\: Normal|)%N F) in H6.
+ assert (exists i:D, 
          i \in ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)
       :&: Normal) :|:
      (Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
       :&: Normal))).
  { apply /card_gt0P. rewrite cardsU.
    assert ((#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1) :&: Normal
               :&: (Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)
                    :&: Normal)|)%N = 0%N).
    { rewrite setIACA. rewrite setIid.
      apply /eqP. rewrite cards_eq0. apply /eqP. apply eq_finset.
      ...
      apply /nandP. left. rewrite inE. 
      rewrite inE.
      assert ((i \in Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) (F + 1)) = 
                ((i \in X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) && 
                  (F + 1 <=
                    #|in_neighbor i :\: X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)|)%N)).
      { by rewrite inE. } rewrite H9.
      rewrite and_distr. apply /nandP. left.
      apply /nandP.
      assert ([disjoint (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) & (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j))]).
      { by apply X_M_X_m_disjoint_at_j. }
      apply (@implies_simp (i \in (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)))
                (i \notin X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j))).
      intros.
      assert ((i \in (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) = false) ->
             i \notin (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j))). 
      { intros. by apply ssrbool.negbT. }
      apply H12. 
      by apply (@disjointFr D (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) i). 
    } by rewrite H9 subn0.
  } destruct H9 as [ i H9].
  exists i.
  assert ( i \in (Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j))
                    (F + 1) :&: Normal) \/ 
           i \in (Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j))
                    (F + 1) :&: Normal)).
  { apply /orP. by rewrite -in_setU. }
  destruct H10.
  - left.
    ...
    rewrite inE in H11.
    assert ((i \in X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) /\
               (F + 1 <=
                #|in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)|)%N).
...
+ apply adversary_max.
  ...
  - apply eq_finset. unfold eqfun. move=>i.
    ...
    apply /nandP. apply implies_simp.
    intros. apply ssrbool.negbT.
    apply (@disjointFr D (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)) i).
    * by apply X_M_X_m_disjoint_at_j.
    * by []. 
Qed.
...
Proof.
intros.
assert ( exists i:D, i \in (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)) /\
                        i \in Normal).
...
+ split.
  ...
  - rewrite eqn_leq in H8. apply split_ineq in H8.
    ...
    assert ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)) \subset 
              (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j))).
    ...
    assert ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1) \subset
               X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) && 
            ((#|X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)| <=
                #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1)|)%N)).
    { apply /andP. by split. }
    assert (Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1) == 
                  X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)).
    { by rewrite eqEcard. }
    ...
    assert ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1) \subset
                  X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) /\
                 (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j) \subset
                  Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1))).
    { by apply /andP. } destruct H15 as [H15 H16].
    rewrite subsets_disjoint in H16.
    apply (@disjointFr D (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j))
              (~: (Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)) (F + 1))) i) in H16.
...
Proof.
intros.
assert ((0 < #|X_M_t_e_i eps_0 A_M t_eps|)%N). { by apply X_M_card_gt_0. }
assert ((0 < #|X_m_t_e_i eps_0 A_m t_eps|)%N). { by apply X_m_card_gt_0. }
assert (X_M_t_e_i eps_0 A_M t_eps \proper Vertex).
{ apply (@X_M_mem t_eps A_M A_m eps_0).
...
+ assert ((0<=j)%N). { by []. } rewrite leq_eqVlt in H22.
  ...
  - assert (j=0%N). { rewrite eq_sym in H23. by apply /eqP. }
   ...
   - specialize (IHj H23). 
     ...
     destruct H5.
     specialize (H27 H6).
     specialize (H27 (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j))
                     (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j))).
     ...
     { split.
       + apply X_M_mem_j with A_m.
         ...
         - apply /card_gt0P.
           assert ( exists i:D, (i \in X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)) /\
                        (i \in Normal)).
           { assert (exists i:D, i \in (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)) :&: Normal).
      ...
      - exists k. split.
        ...
        * rewrite inE. apply /nandP. left.
          ...
          destruct (Rgt_dec (x (t_eps + j) k)
                            (A_M - eps_j j eps_0 eps a)).
          ++ simpl in H31. 
             destruct (Rgt_dec (x (t_eps + j.+1) k)
                              (A_M - eps_j j.+1 eps_0 eps a)).
             - simpl. 
               assert ((x (t_eps + j.+1) k <= A_M -
       ...
       - apply /subsetP. rewrite /sub_mem //=. move=> p H36.
         ...
         assert ((p \in X_M_t_e_i (a * eps_j j eps_0 eps a - (1 - a) * eps) A_M
                            (t_eps + j.+1)) /\ (p \in Normal)).
         { by apply /andP. } destruct H37.
         rewrite in_setI. apply /andP. split.
         * rewrite inE. rewrite inE in H37.
           destruct (Rgt_dec (x (t_eps + j.+1) p)
                          (A_M - (a * eps_j j eps_0 eps a - (1 - a) * eps))).
           ++ simpl in H37.  
              destruct (Rgt_dec (x (t_eps + j) p)
                                (A_M - eps_j j eps_0 eps a)).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x (t_eps + j.+1) p <= A_M -
                                  (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re).
                { apply x_val_decreases_1 with A_m N.
                  + by [].
                  + by []. 
     ...
     * right. destruct H31. destruct H32.
        ...
        * rewrite inE. apply /nandP. left.
          ...
          destruct (Rlt_dec (x (t_eps + j) k)
                            (A_m + eps_j j eps_0 eps a)).
          ++ simpl in H31. 
             destruct (Rlt_dec (x (t_eps + j.+1) k)
                              (A_m + eps_j j.+1 eps_0 eps a)).
             - simpl. 
               assert ((x (t_eps + j.+1) k >= A_m +
                                  (a * eps_j j eps_0 eps a -(1 - a) * eps))%Re).
       ...
       - apply /subsetP. rewrite /sub_mem //=. move=> p H36.
         ...
         rewrite in_setI. apply /andP. split.
         * rewrite inE. rewrite inE in H37.
           destruct (Rlt_dec (x (t_eps + j.+1) p)
                          (A_m + (a * eps_j j eps_0 eps a - (1 - a) * eps))).
           ++ simpl in H37.  
              destruct (Rlt_dec (x (t_eps + j) p)
                                (A_m + eps_j j eps_0 eps a)).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x (t_eps + j.+1) p >= A_m +
...
Lemma sufficiency_proof:
  ...
  (r_s_robustness (F + 1) (F + 1) ->
    Resilient_asymptotic_consensus).
Proof.
intros.
unfold Resilient_asymptotic_consensus.
intros.
rewrite /F_total_malicious in H.
split.
+ assert ((forall t:nat, (M t.+1 <= M t)%Re) -> 
            exists A_M: R, is_lim_seq (fun t: nat => M t) A_M).
  { by apply A_M_exists. }
  ...
  intros.
   ...
   assert (forall (A_m A_M: R),
                ...
                A_m <> A_M ->
                (A_m < A_M)%Re).
   { apply A_m_lt_A_M. } destruct H.
   specialize (H12 A_m A_M H H1 H5 H4 H10).
   ...
   **)
   assert (exists t:nat,  (M t < A_M)%Re \/ (m t > A_m)%Re).
   { assert (exists T:nat, (T<=N)%N).
     { exists (N)%N. by []. }
    
     destruct H0 as [a H0]. destruct H0 as [a_0 H0]. destruct H0 as [a_1 H0].
     ...
     { specialize (H2 (X_M_t_e_i eps_0 A_M t_eps) (X_m_t_e_i eps_0 A_m t_eps)).
       by apply H2.
     }
    ...
    { rewrite HeqN. apply r_s_robustness_implies.
      ...
      + by [].
      + rewrite -HeqN. by [].
      + intros. specialize (H0 t i). (*specialize (H0 t i H17). *) destruct H0.
    ...
    { rewrite HeqN. apply normal_bound_exists.
      ...
      + by [].
      + rewrite -HeqN. by [].
      + intros. specialize (H0 t i). (*specialize (H0 t i H18) *) destruct H0.
      ...
      + apply r_s_robustness_cond_implies.
        ...
        - by [].
        - apply H16.
          * assert ((0 < #|X_M_t_e_i eps_0 A_M t_eps|)%N). 
            { apply X_M_card_gt_0.
              + by [].
              + by [].
              + rewrite /wts_well_behaved. by exists a.
   ...
   assert (forall t : nat, (m t <= A_m)%Re). 
   { by apply m_A_m_lower_bound. }
   specialize (H11 t). specialize (H15 t).
   assert ((M t >= A_M)%Re /\ (m t <= A_m)%Re ).
   { split.
     + by apply Rle_ge.
     + by [].
   }
