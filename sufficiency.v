...
Lemma x_bound: forall (i:D) (A: D -> bool) (mal : nat -> D -> R) (init: D -> R) (w : nat -> D *D -> R)  , 
  i \in (Normal A) ->  
  forall t : nat, (m mal init A w t <= x mal init A w t i <= M mal init A w t)%Re.
Proof.
intros. 
(** x mal init A w t i <= Rmax (x mal init A w t i) (M - (x mal init A w t i))
    Rmin (x mal init A w t i) (m - (x mal init A w t i)) <= x mal init A w t i
**)
split.
...
Lemma order_m (A: D -> bool) (mal : nat -> D -> R) (init : D -> R) (w: nat -> D * D ->R): 
  F_totally_bounded A ->
  (0 < F + 1 <= #|D|)%N -> 
  forall t : nat, (m mal init A w t <= M mal init A w t)%Re.
Proof.
intros. rewrite /m  /M.
apply Rle_trans with 
  (-(nth 0%Re [seq (- x mal init A w  t i)%Ri | i <- enum (Normal A)] (size (enum (Normal A))).-1))%Re.
+ apply Ropp_le_contravar. apply /RleP.
  apply (@bigmaxr_ler _ 0%Re [seq - x mal init A w t i0 | i0 <- enum (Normal A)] (size (enum (Normal A))).-1).
  rewrite size_map. rewrite ltn_predL. by apply size_normal_gt_0.
+ assert ((- nth 0 [seq (- x mal init A w t i)%Ri | i <- enum (Normal A)]
              (size (enum (Normal A))).-1)%Re = 
          nth 0 [seq x mal init A w t i | i <- enum (Normal A)]
              (size (enum (Normal A))).-1).
  { rewrite -(@nth_map R 0%Re R 0%Re (fun x:R => (- x)%Re) 
          (size (enum (Normal A))).-1 [seq (- x mal init A w t i)%Ri | i <- enum (Normal A)]).
    + assert ([seq (- x)%Re | x <- [seq - x mal init A w t i | i <- enum (Normal A)]] = 
                [seq x mal init A w t i | i <- enum (Normal A)]).
      { rewrite -map_comp.
        apply eq_in_map. 
        rewrite /eqfun //=.
        assert ( (forall i:D, i \in (enum (Normal A)) -> 
                  (- ( - (x mal init A w t i))%Ri)%Re = x mal init A w t i) <-> 
                  {in enum (Normal A),
                    forall y : D,
                    (- (-x mal init A w t y)%Ri)%Re = x mal init A w t y}).
...
Lemma interval_bound (A: D -> bool) (mal : nat -> D -> R) (init :D -> R) (w: nat -> D * D -> R):
  F_total_malicious mal init A w ->
  wts_well_behaved A mal init w  ->
  (0 < F + 1 <= #|D|)%N -> 
  forall t : nat,
  (m mal init A w 0 <= m mal init A w t)%Re /\ (M mal init A w t <= M mal init A w 0)%Re.
Proof.
intros. induction t.
  - nra.
  - destruct IHt as [mIHt MIHt].
    ...
    * apply Rle_trans with (m mal init A w t)%Re.
      ++ apply mIHt.
      ++ assert (m mal init A w t.+1 = -(bigmaxr 0 ((map (fun i:D => -(x mal init A w t.+1 i))) (enum (Normal A))))).
         ...
         - intros. rewrite size_map in H3.
           ...
           assert (forall (i:D) (t:nat) (mal : nat -> D -> R) (init: D ->R) (A: D -> bool) (w: nat -> D * D -> R),
                    ...
                    wts_well_behaved A mal init w ->
                    i \in Normal A ->
                    ((x mal init A w (t+1) i <= M mal init A w t)%Re /\ (m mal init A w t <= x mal init A w (t+1) i)%Re)).
           { apply lem_1. }
...
Lemma A_M_exists (A: D -> bool) (mal : nat -> D -> R) (init: D -> R) (w : nat -> D *D -> R):
  F_total_malicious mal init A w ->
  wts_well_behaved A mal init w ->
  (0 < F + 1 <= #|D|)%N ->
  (forall t:nat, (M mal init A w t.+1 <= M mal init A w t)%Re) -> 
  exists A_M: R, is_lim_seq (M mal init A w) A_M.
Proof.
intros. 
...
apply H3 with (m mal init A w 0%N).
+ apply H2.
+ intros. apply Rle_trans with (m mal init A w t).
...
Definition X_M_t_e_i (e_i: R) (A_M:R) (t:nat) (mal : nat -> D ->R) 
  ...
  [set i:D | Rgt_dec (x mal init A w t i) (A_M - e_i)%Re].
...
Lemma X_M_X_m_disjoint (mal : nat -> D ->R) (init : D -> R) (A: D -> bool)  (w: nat -> D * D -> R):
  forall (t:nat) (A_M A_m :R) (eps_0:posreal),
  (A_M - eps_0 > A_m + eps_0)%Re ->
  [disjoint (X_M_t_e_i eps_0 A_M t mal init A w) & (X_m_t_e_i eps_0 A_m t mal init A w)].
Proof.
...
destruct (Rgt_dec (x mal init A w t i) (A_M - eps_0)).
+ simpl. 
  destruct (Rlt_dec (x mal init A w t i) (A_m + eps_0)).
  - simpl. left.
    assert ((x mal init A w t i > A_m + eps_0)%Re).
    { apply Rgt_trans with (A_M - eps_0)%Re; by []. }
    assert ((x mal init A w t i > A_m + eps_0)%Re -> 
            ~(x mal init A w t i < A_m + eps_0)%Re). { nra. }
    specialize (H1 H0).
    contradiction.
  - simpl. by right.
+ simpl. by left.
...
Lemma each_elem_disjoint (mal : nat ->D->R) (init : D -> R) (A: D -> bool) (w : nat -> D * D -> R):
  forall (t:nat) (A_M A_m :R) (eps_0:posreal),
  (A_M - eps_0 > A_m + eps_0)%Re -> 
  (0 < #|X_m_t_e_i eps_0 A_m t mal init A w|)%N ->
  [disjoint
   [set nth key (enum (X_m_t_e_i eps_0 A_m t mal init A w)) 0]
    & X_M_t_e_i eps_0 A_M t mal init A w].
Proof.
...
Lemma X_M_mem (mal : nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  forall (t_eps:nat) (A_M A_m :R) (eps_0:posreal),
  (0 < #|D|)%N ->  (A_M - eps_0 > A_m + eps_0)%Re -> 
  (0 < #|X_m_t_e_i eps_0 A_m t_eps mal init A w|)%N -> 
  X_M_t_e_i eps_0 A_M t_eps mal init A w \proper Vertex.
Proof.
intros.
rewrite properEcard.
apply /andP. split.
...
- rewrite cardsCs.
  ...
  * rewrite card_gt0. apply /set0Pn.
    exists (nth key (enum (X_m_t_e_i eps_0 A_m t_eps mal init A w)) 0%N).
...
- rewrite cardsCs.
  ...
  * rewrite card_gt0. apply /set0Pn.
    ...
    apply single_element_disjoint.
    ++ by [].
    ++ by apply (@X_M_X_m_disjoint mal init A w t_eps A_M A_m eps_0).
  * apply H.
Qed.


Lemma A_M_monotonic (mal: nat -> D ->R) (init : D-> R) (A: D -> bool) (w: nat -> D * D -> R): 
  F_total_malicious mal init A w ->
  wts_well_behaved A mal init w ->
 (0 < F + 1 <= #|D|)%N ->
  forall t : nat, (M mal init A w t.+1 <= M mal init A w t)%Re.
...
intros.
assert (M mal init A w t.+1 = bigmaxr 0 (map (x mal init A w t.+1) (enum (Normal A)))).
{ by rewrite /M. } rewrite H2.
apply bigmax_le.
+ rewrite size_map. destruct H. by apply size_normal_gt_0.
+ intros.
  assert ( nth 0%Re ((map (fun i:D => (x mal init A w t.+1 i))) (enum (Normal A))) i= 
               (fun i:D => (x mal init A w t.+1 i)) (nth key (enum (Normal A)) i)).
  { apply (@nth_map D key R 0%Re (fun i:D => (x mal init A w t.+1 i)) i (enum (Normal A))). 
...
Lemma A_m_monotonic (A: D -> bool) (mal : nat -> D -> R) (init: D -> R) (w: nat -> D * D -> R):
  F_total_malicious mal init A w ->
  wts_well_behaved A mal init w ->
  (0 < F + 1 <= #|D|)%N ->
  forall t:nat, (m mal init A w t.+1 >= m mal init A w t)%Re.
...
{ by rewrite /m. }
rewrite H2.
assert ( m mal init A w t = (- (- (m mal init A w t)))%Re).
...
+ intros.
  ...
  assert (forall (i:D) (t:nat) (mal : nat ->D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R), 
              ...
              i \in (Normal A) ->
               ...
               (m mal init A w t <= x mal init A w (t+1) i)%Re)).
  { apply lem_1. }
  specialize (H6 (nth key (enum (Normal A)) i)).
  specialize (H6 t mal init A w H H0).
  assert (nth key (enum (Normal A)) i \in (Normal A)). 
...
Theorem sandwich_func (mal :nat ->D ->R) (init: D ->R) (A: D -> bool) (w: nat -> D * D -> R):
 forall (u v : nat -> R),
(exists L:Rbar,  
   forall (i:D), i \in (Normal A) ->
   (forall t:nat, ((u t) <= (x mal init A w t i) <= (v t))%Re) /\ 
...
apply (is_lim_seq_le_le 
        (fun t: nat => u t) (fun t: nat => x mal init A w t i)
        (fun t: nat => v t)).
+ destruct H as [H H1].
...
Lemma A_m_lt_A_M (mal : nat -> D ->R) (init: D -> R) (A: D -> bool) (w: nat -> D * D -> R):
   forall (A_m A_M: R),
   F_totally_bounded A ->
  (0 < F + 1 <= #|D|)%N ->
  is_lim_seq (fun t:nat => m mal init A w t) A_m ->
  is_lim_seq (fun t:nat => M mal init A w  t) A_M ->
  A_m <> A_M ->
  (A_m < A_M)%Re.
Proof.
intros.
assert ((forall t:nat, (m mal init A w t <= M mal init A w t)%Re) ->
        is_lim_seq (fun t: nat => m mal init A w t) A_m -> 
        is_lim_seq (fun t: nat => M mal init A w  t) A_M ->
        (A_m <= A_M)%Re).
{ apply is_lim_seq_le. } 
...
Lemma sandwich_x (mal : nat -> D -> R) (init : D -> R) (A : D -> bool) (w: nat -> D * D -> R):
   forall (A_M A_m:R),
  is_lim_seq (fun t:nat => m mal init A w t ) A_m ->
  is_lim_seq (fun t:nat => M mal init A w t) A_M ->
  A_M = A_m -> 
  exists L : Rbar,
    forall i : D,
    i \in Normal A -> is_lim_seq ((x mal init A w)^~ i) L.
Proof.
intros. apply sandwich_func with (fun t:nat => m mal init A w t)
       (fun t:nat => M mal init A w t) .
...
Lemma M_A_M_lower_bound (mal : nat -> D ->R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  forall (A_M : R),
  F_total_malicious mal init A w ->
  wts_well_behaved A mal init w ->
  (0 < F + 1 <= #|D|)%N ->
  is_lim_seq (fun t:nat => M mal init A w t) A_M ->
  forall t:nat, (A_M <= M mal init A w t)%Re.
Proof.
intros.
apply (is_lim_seq_decr_compare (fun t:nat => M mal init A w t) A_M).
+ apply H2. 
...
Lemma m_A_m_lower_bound  (mal : nat ->D->R) (init: D -> R) (A : D -> bool) (w: nat -> D * D -> R):
 forall (A_m : R),
  ...
  (0 < F + 1 <= #|D|)%N ->
  is_lim_seq (fun t:nat => m mal init A w t) A_m ->
  forall t:nat, (m mal init A w t <= A_m)%Re.
Proof.
intros.
apply (is_lim_seq_incr_compare (fun t:nat => m mal init A w t) A_m).
+ apply H2. 
...
Lemma max_bound (A_M:R) (mal : nat ->D ->R) (init: D ->R) (A : D -> bool) (w: nat -> D * D -> R):
  F_totally_bounded A  ->
  (0 < F + 1 <= #|D|)%N ->
  (exists t:nat , forall (i:D), 
    i \in Normal A -> (x mal init A w t i < A_M)%Re) ->
   exists t:nat, (M mal init A w  t < A_M)%Re.
Proof.
intros. destruct H1 as [t H1]. exists t.
rewrite /M. apply bigmax_lt.
+ rewrite size_map. destruct H. by apply size_normal_gt_0.
+ intros. rewrite size_map in H2.
  specialize (H1 (nth key (enum (Normal A)) i)).
  assert (nth key (enum (Normal A)) i \in (Normal A) ).
  { by apply nth_normal. } specialize (H1 H3).
  assert (x mal init A w t (nth key (enum (Normal A)) i) = 
          [seq x mal init A w t i | i <- enum (Normal A)]`_i ).
  { assert ( [seq x mal init A w t i | i <- enum (Normal A)]`_i = 
                 (fun i:D => (x mal init A w t i)) (nth key (enum (Normal A)) i)).
    { by apply nth_map. } by rewrite H4.
  } rewrite -H4. by apply H1.
Qed.

Lemma min_bound (A_m:R) (mal : nat -> D ->R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
   F_totally_bounded A ->
  (0 < F + 1 <= #|D|)%N ->
  (exists t:nat , forall (i:D), 
    i \in Normal A -> (x mal init A w t i > A_m)%Re) ->
...
+ intros. rewrite size_map in H3.
  ...
  { assert ( [seq - (x mal init A w t i) | i <- enum (Normal A)]`_i = 
                 (fun i:D => - (x mal init A w t i)) (nth key (enum (Normal A)) i)).
    { by apply (@nth_map D key R 0%Re (fun i:D => - (x mal init A w t i)) i (enum (Normal A))) . }
    by rewrite H5.
...
Lemma X_M_normal_exists (t_eps: nat) (A_M: R) (eps_0:posreal)
  ...
  (0 < F + 1 <= #|D|)%N ->
   is_lim_seq [eta M mal init A w] A_M ->
    exists i:D, i \in (X_M_t_e_i eps_0 A_M t_eps mal init A w) /\ i \in Normal A.
Proof.
intros. 
assert (M mal init A w t_eps \in map (fun i:D => x mal init A w t_eps i) (enum (Normal A))).
{ rewrite /M. 
  apply (@bigmaxr_mem _ 0%Re (map (fun i:D => x mal init A w t_eps i) (enum (Normal A)))).
  rewrite size_map. destruct H. by apply size_normal_gt_0.
} 
assert (exists2 i:D, i \in (enum (Normal A)) &
            (M mal init A w t_eps = (fun i:D => x mal init A w t_eps i) i)).
{ by apply /mapP. }
destruct H4 as [i H4]. exists i.
split.
+ rewrite inE. rewrite -H5.
  destruct (Rgt_dec (M mal init A w t_eps) (A_M - eps_0)).
  - by simpl.
  - simpl. 
    assert ((A_M - eps_0 < M mal init A w t_eps)%Re).
...
intros. 
assert ((-(m mal init A w t_eps)) \in map (fun i:D => - x mal init A w t_eps i) (enum (Normal A))).
{ rewrite /m. rewrite -RoppE. rewrite Ropp_involutive.
  apply (@bigmaxr_mem _ 0%Re (map (fun i:D => - x mal init A w t_eps i) (enum (Normal A)))).
  rewrite size_map. destruct H. by apply size_normal_gt_0.
} 
assert (exists2 i:D, i \in (enum (Normal A)) &
...
split.
+ rewrite inE.
  assert (m mal init A w t_eps = x mal init A w t_eps i). 
  { rewrite -[LHS]Ropp_involutive. rewrite !RoppE. rewrite H5.
    by rewrite -!RoppE Ropp_involutive.
  } rewrite -H6.
  destruct (Rlt_dec (m mal init A w t_eps) (A_m + eps_0)).
  - by simpl.
  - simpl. 
    assert ((m mal init A w t_eps < A_m + eps_0)%Re).
...
Lemma x_val_at_neighbor (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal)
  (mal : nat -> D -> R) (init: D -> R) (A : D -> bool) (w: nat -> D * D -> R):
  forall k:D, k \in (in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) ->
  (x mal init A w (t_eps+j)%N k <= A_M - (eps_j j eps_0 eps a))%Re.
Proof.
intros. rewrite in_setD in H.
assert ((k \notin X_M_t_e_i (eps_j j eps_0 eps a) A_M
                (t_eps + j)%N mal init A w) /\ (k \in in_neighbor i)).
{ by apply /andP. } destruct H0. rewrite inE in H0.
destruct (Rgt_dec (x mal init A w (t_eps + j)%N k)
...
Lemma x_left_ineq_1 (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal)
  ...
  (x mal init A w (t_eps + j)%N i <= A_M - eps_j j eps_0 eps a)%Re ->
  ...
  (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
    ...
    (x mal init A w (t_eps + j)%N (nth i incl i0) *
     w (t_eps + j)%N (i, nth i incl i0))%Re <=
 (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
      (i0 < (@inord  (size incl - 1) (index i incl)))%N)
     w (t_eps + j)%N (i, nth i incl i0)) *
    (A_M - eps_j j eps_0 eps a))%Re.
Proof.
intros. rewrite (@big_cond_simpl_lt i incl (fun i0:nat => (x mal init A w (t_eps + j)%N (nth i incl i0) *
                    w (t_eps + j)%N (i, nth i incl i0))%Re)).
rewrite (@big_cond_simpl_lt i incl (fun i0:nat =>  w (t_eps + j)%N (i, nth i incl i0))).
apply /RleP.
...
+ apply Rle_trans with a.
  ...
  - apply H3. apply mem_nth.
    ...
    ++ by apply ltn_ord.
    ++ assert ((0< size incl)%N). { apply size_incl_not_0. }
       rewrite ltn_subrL. apply /andP. split.
       - by [].
       - apply H6.
+ apply Rle_trans with (x mal init A w (t_eps + j)%N i).
  - assert (sorted_Dseq (x mal init A w (t_eps + j)%N) (incl_neigh_minus_extremes i (x mal init A w (t_eps + j)%N))).
    { apply lemma_1.incl_sorted. } rewrite /sorted_Dseq in H6.
    apply H6.
  ...
  - rewrite index_uniq.
    * rewrite -/incl. rewrite inordK in H5.
      ...
      ++ rewrite subn1. rewrite prednK.
         - rewrite index_mem. apply i_in_incl.
...
Lemma R_i_gt_val_rel (i:D) (t:nat) (mal: nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  let incl :=  incl_neigh_minus_extremes i (x mal init A w t) in
  forall j k:D, 
    k \in incl -> 
     j \in (R_i_greater_than mal init A w i t) ->
      (x mal init A w t k <= x mal init A w t j)%Re.
Proof.
intros. rewrite /R_i_greater_than in H0.
rewrite inE in H0. apply and_conv in H0. destruct H0.
destruct H1.
destruct (Rgt_dec (x mal init A w t j) (x mal init A w t i)).
+ simpl in H0.
  rewrite /incl / incl_neigh_minus_extremes /remove_extremes in H.
  rewrite mem_filter in H. apply and_conv in H. destruct H.
  destruct H3.
  assert (Rle_dec (x mal init A w t k) (x mal init A w t i)
             \/ (index k (inclusive_neighbor_list i (x mal init A w t)) <=
                 size (inclusive_neighbor_list i (x mal init A w t)) - F - 1)%N).
  { by apply /orP. } destruct H5.
  - destruct (Rle_dec (x mal init A w t k) (x mal init A w t i)).
    * simpl in H5. apply Rle_trans with (x mal init A w t i).
      ++ apply r0.
      ++ by apply Rlt_le.
    * by simpl in H5.
  - assert(sorted_Dseq (x mal init A w t) (inclusive_neighbor_list i (x mal init A w t))).
    { by apply lemma_1.inclusive_neighbors_sorted_0. } rewrite /sorted_Dseq in H6.
    apply H6.
    - apply H4.
    - apply H2.
    - by apply leq_ltn_trans with (size (inclusive_neighbor_list i (x mal init A w t)) - F - 1)%N.
+ by simpl in H0.
Qed.
...
Lemma exists_an_elem_in_out_set (i:D) (t:nat)
  ...
  nth i incl i0 \in Adversary A ->
  exists k:D, k \in (Normal A :&: (R_i_greater_than mal init A w i t)).
Proof.
intros.
assert (exists (j:D), (j \in Normal A) /\ (j \in R_i_greater_than mal init A w i t)).
...
Lemma not_in_R_i_greater_than (i j:D) (t:nat)
  (mal : nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  let incl :=  incl_neigh_minus_extremes i (x mal init A w t) in
    (j \in incl \/ j \in R_i_less_than mal init A w i t) ->
...
+ rewrite inE. rewrite /incl /incl_neigh_minus_extremes /remove_extremes in H.
  ...
  - left. destruct (Rle_dec (x mal init A w t j) (x mal init A w t i)).
    * simpl in H2. 
      destruct (Rgt_dec (x mal init A w t j) (x mal init A w t i)).
...
+ rewrite inE. rewrite inE in H. apply and_conv in H.
  ...
  apply /nandP. left.
  destruct (Rlt_dec (x mal init A w t j) (x mal init A w t i)).
  - destruct (Rgt_dec (x mal init A w t j) (x mal init A w t i)).
...
Lemma x_right_ineq_1 (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal)
 ...
 (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
    ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
       w (t_eps + j)%N (i, nth i incl i0)) * M mal init A w (t_eps + j)%N)%Re.
Proof.
intros. rewrite (@big_cond_simpl_gt i incl (fun i0:nat => (x mal init A w (t_eps + j)%N (nth i incl i0) *
                    w (t_eps + j)%N (i, nth i incl i0))%Re)).
rewrite (@big_cond_simpl_gt i incl (fun i0:nat =>  w (t_eps + j)%N (i, nth i incl i0))).
apply /RleP.
assert (((\sum_(i0 < (size incl - 1).+1 | 
              ((@inord  (size incl - 1) (index i incl)) < i0)%N)
                 w (t_eps + j)%N (i, nth i incl i0)) *
             M mal init A w (t_eps + j)%N)%Re = 
          \sum_(i0 < (size incl - 1).+1 | 
             ((@inord  (size incl - 1) (index i incl)) < i0)%N)
                 ((w (t_eps + j)%N (i, nth i incl i0)) *
             M mal init A w (t_eps + j)%N)%Re).
...
+ apply Rle_trans with a.
  ...
  - apply H3. apply mem_nth.
    ...
    ++ assert ((0< size incl)%N). { apply size_incl_not_0. }
       ...
       - by [].
       - apply H6.
+ assert ((nth i incl i0) \in Normal A \/ (nth i incl i0) \in Adversary A).
  { apply normal_or_adversary. }
  destruct H6.
  - by apply x_bound.
  - assert ((#|R_i_greater_than mal init A w i (t_eps + j)%N| <= F)%N).
    { apply R_i_gt_bounded. } rewrite leq_eqVlt in H7.
    assert ( (#|R_i_greater_than mal init A w i (t_eps + j)%N| == F)
              \/ (#|R_i_greater_than mal init A w i (t_eps + j)%N| < F)%N).
    { by apply /orP. } destruct H8.
    * assert(R_i_greater_than mal init A w i (t_eps + j)%N == Adversary A \/
                  R_i_greater_than mal init A w i (t_eps + j)%N != Adversary A).
      { apply /orP.
        by destruct (R_i_greater_than mal init A w i (t_eps + j)%N == Adversary A).
      } destruct H9.
      ++ assert (R_i_greater_than mal init A w i (t_eps + j)%N == Adversary A ->
                  nth i incl i0 \notin Adversary A).
         { by apply incl_does_not_contain_adversary. } specialize (H10 H9). 
         assert (~ (nth i incl i0 \in Adversary A)).
         { by apply /negP. } contradiction.
      ++ assert (exists k:D, k \in (Normal A :&: (R_i_greater_than mal init A w  i (t_eps+j)%N))).
        { by apply exists_an_elem_in_out_set with i0.  } destruct H10 as [k H10]. rewrite in_setI in H10.
        assert ((k \in Normal A) /\
                     (k \in R_i_greater_than mal init A w i (t_eps + j)%N)).
        { by apply /andP. } destruct H11.
        apply Rle_trans with (x mal init A w(t_eps + j)%N k).
    ...
    * (** The last node has the value same as x_i(t) and since i is Normal,
        x_i <= M **)
      apply Rle_trans with (x mal init A w (t_eps + j)%N (last (head i incl) (behead incl))).
      - apply last_incl_is_max. rewrite -/incl. apply mem_nth.
        apply ltn_leq_trans with (size incl - 1).+1.
        + by apply ltn_ord.
        + assert ((0< size incl)%N). { apply size_incl_not_0. }
          rewrite ltn_subrL. apply /andP. split.
          - by [].
          - apply H9.
      - assert (((x mal init A w (t_eps + j)%N (last (head i incl ) (behead incl ))) = (x mal init A w (t_eps + j)%N i))%Re).
        { by apply card_R_i_gt_lt_F_exchange_last. } rewrite H9.
        by apply x_bound.
Qed.
...
Lemma w_ge_a_1 (i:D) (a:R) (t_eps j:nat)
  ...
  (forall k:D, k \in incl -> (a <= w (t_eps + j)%N  (i, k))%Re) ->
  sum_f_R0
    (fun n : nat =>  w (t_eps + j)%N (i, nth i incl n)) (size incl - 1) = 1%Re ->
  (0 < index i incl)%N -> 
  let w_il:= \sum_(i0 < (size incl - 1).+1 | 
          (i0 < @inord (size incl -1) (index i incl))%N)
             w (t_eps + j)%N (i, nth i incl i0) in 
 (a <= w_il)%Re. 
Proof.
intros. rewrite /w_il. 
...
Lemma x_val_decreases_1 (i:D) (a A_m A_M:R) (t_eps j N:nat) (eps eps_0: posreal)
  ...
  (x mal init A w (t_eps + j)%N i <= A_M - eps_j j eps_0 eps a)%Re ->
  (x mal init A w (t_eps + j.+1)%N i <=  A_M - eps_j j.+1 eps_0 eps a)%Re.
Proof.
intros. simpl. 
assert ((t_eps + j.+1)%N = (t_eps + j)%N.+1).
   ...
   + rewrite nth_index.
     - rewrite -RplusE.
       assert (\sum_(i0 < (size incl - 1).+1 | i0
                                 ...
                                 (index i incl))
                (x mal init A w (t_eps + j)%N (nth i incl i0) *
                 w (t_eps + j)%N (i, nth i incl i0))%Re  = \sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                 ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                    (x mal init A w (t_eps + j)%N (nth i incl i0) *
                     w (t_eps + j)%N (i, nth i incl i0))%Re).
       { apply eq_big.
         + unfold eqfun. intros.
           ...
           - rewrite subn1. rewrite prednK; apply size_incl_not_0.
         + by intros.
       } rewrite H17. clear H17.
       apply Rle_trans with
            (x mal init A w (t_eps + j)%N i * w (t_eps + j)%N (i, i) +
              (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                              ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                  w (t_eps + j)%N (i, nth i incl i0)) * M mal init A w (t_eps+j)%N)%Re.
       ...
       * apply Rle_trans with 
          ((A_M - eps_j j eps_0 eps a) * w (t_eps + j)%N (i, i) +
             (\sum_(i0 < (size incl - 1).+1 | 
         ...
         ++ assert (( A_M - (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
            ...
            { assert (\sum_(i0 < (size incl - 1).+1 | (i0
                         ...
                         w (t_eps + j)%N (i, nth i incl i0) =
                      (1 - w (t_eps + j)%N (i, i))%Re <-> 
                      (w (t_eps + j)%N (i, i) + \sum_(i0 < (size incl - 1).+1 | (i0
                                 != 
                                 @inord (size incl -1)
                                 (index i incl)) &&
                                ~~
                                (i0 <
                                 @inord (size incl -1)
                                 (index i incl))%N)
                         w (t_eps + j)%N (i, nth i incl i0))%Re = 1%Re).
            ...
            assert ((M mal init A w (t_eps+j)%N < A_M + eps)%Re). 
            { apply H11. apply /leP. apply leq_addr. }
            remember  (w (t_eps + j)%N (i, i)) as w_ii.
            apply Rle_trans with ((1 - a) * M mal init A w (t_eps + j)%N +
                                      a * (A_M - eps_j j eps_0 eps a))%Re.
            - assert (((A_M - eps_j j eps_0 eps a) * w_ii +
                        (1 - w_ii) * M mal init A w (t_eps + j)%N)%Re = 
                        (M mal init A w (t_eps + j)%N - w_ii * ( M mal init A w (t_eps + j)%N - (A_M - eps_j j eps_0 eps a)))%Re).
              { nra. } rewrite H20.
                assert (((1 - a) * M mal init A w (t_eps + j)%N +
                            a * (A_M - eps_j j eps_0 eps a))%Re = 
                         (M mal init A w (t_eps + j)%N - a * (M mal init A w (t_eps + j)%N - (A_M - eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H21.
                apply Rplus_le_compat_l, Ropp_ge_le_contravar.
                apply Rmult_ge_compat_r.
                ...
                * apply Rle_ge. rewrite Heqw_ii. apply H9. apply i_in_incl.
...
++ rewrite (bigD1 (inord (index i incl))) //=. 
  ...
  + rewrite nth_index. 
    - rewrite -RplusE. 
      apply Rle_trans with ((M mal init A w  (t_eps +j)%N) * w (t_eps + j)%N (i, i) +
           \sum_(i0 < (size incl - 1).+1 | 
      ...
      * apply Rplus_le_compat_r, Rmult_le_compat_r.
        ++ specialize (H9 i).
           assert (i \in incl). {  apply i_in_incl. }
           specialize (H9 H17).
      ...
      * rewrite -RplusE -RmultE.
        ...
        rewrite (@big_nat_split i incl  
                  (fun i0 :nat => (x mal init A w (t_eps + j)%N (nth i incl i0) *
                      w (t_eps + j)%N (i, nth i incl i0))%Re)).
        apply Rle_trans with
        (M mal init A w (t_eps + j)%N * w (t_eps + j)%N (i, i) +  
          (((\sum_(i0 < (size incl - 1).+1 | 
                (i0 != (@inord  (size incl - 1) (index i incl))) &&
                (i0 < (@inord (size incl - 1) (index i incl)))%N)
                    w (t_eps + j)%N (i, nth i incl i0)) * (A_M - eps_j j eps_0 eps a))%Re +
          (\sum_(i0 < (size incl - 1).+1 | (i0
             ...
             (x mal init A w (t_eps + j)%N (nth i incl i0) *
              w (t_eps + j)%N (i, nth i incl i0))%Re)%Ri))%Re.
        ...
        ++ apply Rle_trans with
            ...
            * assert ((M mal init A w (t_eps + j)%N * w (t_eps + j)%N (i, i) +
                       ((\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j)%N (i, nth i incl i0)) *
                        (A_M - eps_j j eps_0 eps a) +
                        (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j)%N (i, nth i incl i0)) * M mal init A w (t_eps + j)%N))%Re = 
                       ((w (t_eps + j)%N (i, i) + (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         ~~ (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j)%N (i, nth i incl i0))) * M mal init A w (t_eps+j)%N + 
                        (\sum_(i0 < (size incl - 1).+1 | (i0 != (@inord  (size incl - 1) (index i incl))) &&
                                                         (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j)%N (i, nth i incl i0)) *
                        (A_M - eps_j j eps_0 eps a))%Re).
              ...
              { assert ((w (t_eps + j)%N (i, i) +
                        ...
                        (w (t_eps + j)%N (i, i) +
                         \sum_(i0 < (size incl - 1).+1 | 
                         ((@inord  (size incl - 1) (index i incl)) < i0)%N)
                            w (t_eps + j)%N (i, nth i incl i0) + \sum_(i0 < (size incl - 1).+1 | 
                         (i0 < (@inord  (size incl - 1) (index i incl)))%N)
                            w (t_eps + j)%N (i, nth i incl i0))%Re = 1%Re). { nra. }
                ...
                + rewrite nth_index.
                  - rewrite Rplus_assoc. rewrite -[in RHS]RplusE.
                    ...
                    rewrite [in RHS](@big_nat_split i incl 
                        (fun i0:nat => w (t_eps + j)%N (i, nth i incl i0))).
              ...
              remember (\sum_(i0 < (size incl - 1).+1 | 
                          (i0 < inord (index i incl))%N)
                             w (t_eps + j)%N (i, nth i incl i0)) as w_il.
              assert ((M mal init A w (t_eps+j)%N < A_M + eps)%Re). 
              { apply H11. apply /leP. apply leq_addr. }
              apply Rle_trans with ((1 - a) * M mal init A w (t_eps + j)%N +
                                      a * (A_M - eps_j j eps_0 eps a))%Re.
              - assert (((1 - w_il) * M mal init A w (t_eps + j)%N +
                              w_il * (A_M - eps_j j eps_0 eps a))%Re = 
                        (M mal init A w (t_eps + j)%N - w_il * ( M mal init A w (t_eps + j)%N - (A_M - eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H20.
                assert (((1 - a) * M mal init A w (t_eps + j)%N +
                            a * (A_M - eps_j j eps_0 eps a))%Re = 
                         (M mal init A w (t_eps + j)%N - a * (M mal init A w (t_eps + j)%N - (A_M - eps_j j eps_0 eps a)))%Re).
...
Lemma x_in_incl_not_X_M (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal)
  (mal : nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R): 
  forall (k:D),
  (k \in (in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w)) ->
    (x mal init A w (t_eps+j)%N k <= (A_M - eps_j j eps_0 eps a))%Re. 
Proof.
...
Lemma in_neighbor_subset_incl (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal)
  ...
  {subset (in_neighbor i  :\: 
                X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w) <= 
     [set x | x \in incl] :|: R_i_less_than mal init A w i (t_eps+j)%N }.
Proof.
...
rewrite inE. 
destruct (Rgt_dec (x mal init A w (t_eps + j)%N k) (A_M - eps_j j eps_0 eps a)).
+ by simpl in H1.
+ simpl in H1. 
  assert ((F <= index k (inclusive_neighbor_list i (x mal init A w (t_eps + j))))%N \/ (index k (inclusive_neighbor_list i (x mal init A w (t_eps + j))) <=F)%N).
  ...
  - left.
    ...
    * apply /andP. split.
      ++ apply /orP. by right.
      ++ apply /orP. left.
         destruct (Rle_dec (x mal init A w (t_eps + j)%N k) (x mal init A w (t_eps + j)%N i)).
         - by simpl.
         - simpl.
           assert ((x mal init A w (t_eps + j)%N k <= x mal init A w (t_eps + j)%N i)%Re).
    ...
    * rewrite /inclusive_neighbor_list /inclusive_neigh.
      ...
      rewrite mem_sort. by rewrite inE in H4.
  - rewrite leq_eqVlt in H3.
    assert ((index k (inclusive_neighbor_list i (x mal init A w (t_eps + j)%N)) == F)
              \/ (index k (inclusive_neighbor_list i (x mal init A w (t_eps + j)%N)) < F)%N).
    ...
    * assert (index k (inclusive_neighbor_list i (x mal init A w (t_eps + j)%N)) = F).
      ...
      ++ rewrite /inclusive_neighbor_list /inclusive_neigh.
          ...
          rewrite mem_sort. by rewrite inE in H6.
    * right. rewrite inE. apply /andP. split.
      ++ apply /andP. split.
         - destruct (Rlt_dec (x mal init A w (t_eps + j)%N k) (x mal init A w (t_eps + j)%N i)).
           * by simpl.
           * simpl.
             assert ((x mal init A w (t_eps + j)%N k < x mal init A w (t_eps + j)%N i)%Re).
      ...
      ++ rewrite /inclusive_neighbor_list /inclusive_neigh.
          ...
          { apply set_enum. } rewrite -H6 in H5.
           rewrite mem_sort. by rewrite inE in H5.
Qed.
...
Lemma s_subset_incl (i:D) (a A_M:R) (t_eps j:nat) (eps eps_0: posreal) (s: seq D)
  ...
  {subset s <= in_neighbor i  :\: 
                X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w } ->
  {subset s <= [set x | x \in incl] :|: R_i_less_than mal init A w i (t_eps+j)%N}.
Proof.
intros.
assert ({subset [set x | x \in s] <= [set x | x \in incl] :|: R_i_less_than mal init A w i (t_eps+j)%N}).
{ apply /subsetP.
...
Lemma k_in_incl_bounded (i:D) (a A_m A_M:R) (t_eps j N:nat) (eps eps_0: posreal)
  ...
  sum_f_R0
   ...
   (forall t : nat,
     (t >= t_eps)%coq_nat ->
     (m mal init A w t > A_m - eps)%Re /\ (M mal init A w t < A_M + eps)%Re) ->
  (F + 1 <= #|in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w|)%N ->
  (x mal init A w (t_eps + j)%N i > A_M - eps_j j eps_0 eps a)%Re ->
  (forall k: 'I_(size incl - 1).+1,
               (x mal init A w (t_eps + j)%N (nth i incl k) <= M mal init A w (t_eps + j)%N)%Re).
Proof.
intros.
...
+ rewrite leq_eqVlt in H13.
  ...
  - assert (k = inord (index i incl)).
    ...
    * rewrite subn1 prednK.
      ++ rewrite index_mem. apply i_in_incl.
      ++ apply size_incl_not_0.
  - apply Rle_trans with  (x mal init A w (t_eps + j)%N (nth i incl (@inord (size incl -1) (index i incl))))%Re.
    * rewrite inordK.
      ++ rewrite nth_index.
         - assert (sorted_Dseq (x mal init A w (t_eps + j)) incl).
           { apply lemma_1.incl_sorted. } rewrite /sorted_Dseq in H15.
           apply H15.
           * apply mem_nth. apply ltn_leq_trans with (size incl - 1).+1.
             ++ apply ltn_ord.
             ++ rewrite subn1. rewrite ltn_predL. apply size_incl_not_0.
           * apply i_in_incl.
           * rewrite index_uniq.
             ...
             ++ apply incl_uniq.
         - apply i_in_incl.
      ++ rewrite subn1 prednK. rewrite index_mem. apply i_in_incl. apply size_incl_not_0.
    * rewrite inordK.
      ++ rewrite nth_index.
         - by apply x_bound.
         - apply i_in_incl.
      ++ rewrite subn1 prednK. rewrite index_mem. apply i_in_incl. apply size_incl_not_0.
+ assert ((nth i incl k) \in Normal A \/ (nth i incl k) \in Adversary A).
  { apply normal_or_adversary. }
  destruct H14.
  - by apply x_bound.
  - assert ((#|R_i_greater_than mal init A w i (t_eps + j)%N| <= F)%N).
    { apply R_i_gt_bounded . } rewrite leq_eqVlt in H15.
    assert ( (#|R_i_greater_than mal init A w i (t_eps + j)%N| == F)
              \/ (#|R_i_greater_than mal init A w i (t_eps + j)%N| < F)%N).
    { by apply /orP. } destruct H16.
    * assert(R_i_greater_than mal init A w i (t_eps + j)%N == Adversary A \/
      ...
      ++ assert (R_i_greater_than mal init A w i (t_eps + j)%N == Adversary A ->
         ...
         assert (~ (nth i incl k \in Adversary A)).
         { by apply /negP. } contradiction.
      ++ assert (exists k:D, k \in (Normal A :&: (R_i_greater_than mal init A w  i (t_eps+j)%N))).
        { by apply exists_an_elem_in_out_set with k.  } destruct H18 as [p H18]. rewrite in_setI in H18.
        assert ((p \in Normal A) /\
                     (p \in R_i_greater_than mal init A w i (t_eps + j)%N)).
        { by apply /andP. } destruct H19.
        apply Rle_trans with (x mal init A w (t_eps + j)%N p).
        * apply R_i_gt_val_rel with i.
          ++ rewrite -/incl. apply mem_nth.
             apply ltn_leq_trans with (size incl - 1).+1.
             + by apply ltn_ord.
    ...
    * (** The last node has the value same as x_i(t) and since i is Normal,
      ...
      - apply last_incl_is_max. rewrite -/incl. apply mem_nth.
        ...
        + assert ((0< size incl)%N). { apply size_incl_not_0. }
          ...
          - apply H17.
      - assert (((x mal init A w (t_eps + j)%N (last (head i incl ) (behead incl ))) = (x mal init A w (t_eps + j)%N i))%Re).
        { by apply card_R_i_gt_lt_F_exchange_last. } rewrite H17.
        by apply x_bound.
Qed.
...
Lemma x_val_decreases_2 (i:D) (a A_m A_M:R) (t_eps j N:nat) (eps eps_0: posreal)
  ...
  (F + 1 <= #|in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w|)%N ->
  (x mal init A w (t_eps + j)%N i > A_M - eps_j j eps_0 eps a)%Re ->
  (x mal init A w (t_eps + j.+1) i <=  A_M - eps_j j.+1 eps_0 eps a)%Re.
Proof.
intros.  simpl. 
assert ((t_eps + j.+1)%N = (t_eps + j)%N.+1).
...
assert (exists s, [/\ uniq s, size s = (F+1)%N & 
              {subset s <= (in_neighbor i :\: X_M_t_e_i  (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w)}]).
{ by apply /card_geqP. } destruct H16 as [s H16].
destruct H16.
assert (exists k:D, k \in [set x | x \in s] :&: [set x | x \in incl]).
{ apply exists_in_intersection with (R_i_less_than mal init A w i (t_eps+j)%N ) F.
...
+ rewrite nth_index.
    ...
    assert ((\sum_(i0 < (size incl - 1).+1 | 
             i0 != inord (index k incl))
                (x mal init A w (t_eps + j)%N (nth i incl i0) *
                 w (t_eps + j)%N (i, nth i incl i0))%Re <=
            ((\sum_(i0 < (size incl - 1).+1 | 
                i0 != inord (index k incl))  w (t_eps + j)%N (i, nth i incl i0)) *
             M mal init A w (t_eps+j)%N)%Re)%Re).
    { by apply x_not_k_bound with a A_m A_M N eps eps_0 s. }
   apply Rle_trans with 
    (x mal init A w (t_eps + j)%N k * w (t_eps + j)%N (i, k) + 
      ((\sum_(i0 < (size incl - 1).+1 | 
        i0 != inord (index k incl))
           w (t_eps + j)%N (i, nth i incl i0)) *
       M mal init A w (t_eps + j)%N)%Re)%Re.
   ...
   * apply Rle_trans with 
      ( (w (t_eps + j)%N (i, k) * (A_M - eps_j j eps_0 eps a))%Re+
         (\sum_(i0 < (size incl - 1).+1 | 
          i0 != inord (index k incl))
             w (t_eps + j)%N (i, nth i incl i0)) *
         M mal init A w (t_eps + j)%N)%Re.
     ...
     ++ assert (( A_M - (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
        ...
        remember (w (t_eps + j)%N (i, k)) as w_ik.
        assert ((M mal init A w (t_eps+j)%N < A_M + eps)%Re). 
        { apply H11. apply /leP. apply leq_addr. }
        apply Rle_trans with ((1 - a) * M mal init A w (t_eps + j)%N +
                                      a * (A_M - eps_j j eps_0 eps a))%Re.
        - assert ((w_ik * (A_M - eps_j j eps_0 eps a) +
                    (1 - w_ik) * M mal init A w (t_eps + j)%N)%Re = 
                        (M mal init A w (t_eps + j)%N - w_ik * ( M mal init A w (t_eps + j)%N - (A_M - eps_j j eps_0 eps a)))%Re).
          { nra. } rewrite H25.
          assert (((1 - a) * M mal init A w (t_eps + j)%N +
                            a * (A_M - eps_j j eps_0 eps a))%Re = 
                         (M mal init A w (t_eps + j)%N - a * (M mal init A w (t_eps + j)%N - (A_M - eps_j j eps_0 eps a)))%Re).
...
Lemma exists_an_elem_in_out_set_R_lt (i:D) (t:nat)
  ...
  forall i0 : 'I_(size incl - 1).+1, 
  nth i incl i0 \in (Adversary A) ->
  exists k:D, k \in (Normal A :&: (R_i_less_than mal init A w i t)).
Proof.
intros.
assert (exists (j:D), (j \in Normal A) /\ (j \in R_i_less_than mal init A w i t)).
...
Lemma not_in_R_i_less_than (i j:D) (t:nat)
  (mal : nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  let incl :=  incl_neigh_minus_extremes i (x mal init A w t) in
    (j \in incl \/ j \in R_i_greater_than mal init A w i t) ->
...
+ rewrite inE. rewrite /incl /incl_neigh_minus_extremes /remove_extremes in H.
  ...
  assert (Rge_dec (x mal init A w t j) (x mal init A w t i)
             \/ (F <= index j (inclusive_neighbor_list i (x mal init A w t)))%N).
  { by apply /orP. } destruct H2.
  - left. destruct (Rge_dec (x mal init A w t j) (x mal init A w t i)).
    * simpl in H2. 
      destruct (Rlt_dec (x mal init A w t j) (x mal init A w t i)).
  ...
  forall j k:D, 
    k \in incl -> 
     j \in (R_i_less_than mal init A w i t) ->
      (x mal init A w t j <= x mal init A w t k)%Re.
Proof.
intros. rewrite /R_i_less_than in H0.
rewrite inE in H0. apply and_conv in H0. destruct H0.
destruct H1.
destruct (Rlt_dec (x mal init A w t j) (x mal init A w t i)).
+ simpl in H0.
  ...
  rewrite mem_filter in H. apply and_conv in H. destruct H.
  destruct H3.
  assert (Rge_dec (x mal init A w t k) (x mal init A w t i)
             \/ (F <= index k (inclusive_neighbor_list i (x mal init A w t)))%N).
  { by apply /orP. } destruct H5.
  - destruct (Rge_dec (x mal init A w t k) (x mal init A w t i)).
    * simpl in H5. apply Rle_trans with (x mal init A w t i).
      ++ by apply Rlt_le.
      ++ apply Rge_le, r0.
    * by simpl in H5.
...
+ assert ((nth i incl i0) \in Normal A \/ (nth i incl i0) \in Adversary A).
  ...
  destruct H6.
  - by apply x_bound.
  - assert ((#|R_i_less_than mal init A w i (t_eps + j)%N| <= F)%N).
    { apply R_i_lt_bounded. } rewrite leq_eqVlt in H7.
    assert ( (#|R_i_less_than mal init A w i (t_eps + j)%N| == F)
              \/ (#|R_i_less_than mal init A w i (t_eps + j)%N| < F)%N).
    { by apply /orP. } destruct H8.
    * assert(R_i_less_than mal init A w i (t_eps + j)%N == Adversary A \/
                  R_i_less_than mal init A w i (t_eps + j)%N != Adversary A).
      { apply /orP.
        by destruct (R_i_less_than mal init A w i (t_eps + j)%N == Adversary A).
      } destruct H9.
      ++ assert (R_i_less_than mal init A w i (t_eps + j)%N == Adversary A ->
                  nth i incl i0 \notin Adversary A).
         ...
         assert (~ (nth i incl i0 \in Adversary A)).
         { by apply /negP. } contradiction.
      ++ assert (exists k:D, k \in (Normal A :&: (R_i_less_than mal init A w  i (t_eps+j)%N))).
        { by apply exists_an_elem_in_out_set_R_lt with i0.  } destruct H10 as [k H10]. rewrite in_setI in H10.
        assert ((k \in Normal A) /\
                     (k \in R_i_less_than mal init A w i (t_eps + j)%N)).
        { by apply /andP. } destruct H11.
        apply Rle_trans with (x mal init A w (t_eps + j)%N k).
        * by apply x_bound.
    ...
    * (** The last node has the value same as x_i(t) and since i is Normal,
        x_i <= M **)
      apply Rle_trans with ((x mal init A w (t_eps + j)%N) (nth i (incl_neigh_minus_extremes i (x mal init A w (t_eps + j)%N)) 0)).
      - assert(((x mal init A w (t_eps+j)%N (nth i (incl_neigh_minus_extremes i (x mal init A w (t_eps + j)%N)) 0)) = (x mal init A w (t_eps + j)%N i))%Re).
        { by apply  card_R_i_lt_lt_F_exchange_first. } rewrite H9.
        by apply x_bound.
      - apply first_incl_is_min. rewrite -/incl. apply mem_nth.
        apply ltn_leq_trans with (size incl - 1).+1.
        + by apply ltn_ord.
        + assert ((0< size incl)%N). { apply size_incl_not_0. }
          rewrite ltn_subrL. apply /andP. split.
         ...
         + unfold eqfun. intros. 
           ...
           { apply leq_gtF. rewrite inordK.
             + apply ltnSE. rewrite prednK.
               - apply ltn_leq_trans with (size incl -1).+1. 
                 * apply ltn_ord.
                 * rewrite subn1 ltn_predL. apply size_incl_not_0.
               - apply size_incl_not_0.
             + rewrite subn1. rewrite prednK.
               - by [].
               - apply size_incl_not_0.
       ...
       * apply Rge_trans with 
         ...
         ++ assert (( A_m + (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
            ...
            { assert ((\sum_(i0 < (size incl - 1).+1 | 
              ...
              assert ((m mal init A w (t_eps+j)%N > A_m - eps)%Re). 
              { apply H11. apply /leP. apply leq_addr. }
              apply Rge_trans with ((1 - a) * m mal init A w (t_eps + j)%N +
                                      a * (A_m + eps_j j eps_0 eps a))%Re.
              - assert (((A_m + eps_j j eps_0 eps a) * w_ii +
                          (1 - w_ii) * m mal init A w (t_eps + j)%N)%Re = 
                        (m mal init A w (t_eps + j)%N - w_ii * ( m mal init A w (t_eps + j)%N - (A_m + eps_j j eps_0 eps a)))%Re).
                ...
                assert ((- (w_ii * (m mal init A w (t_eps + j)%N - (A_m + eps_j j eps_0 eps a))))%Re = 
                          (w_ii * ((A_m + eps_j j eps_0 eps a) - m mal init A w (t_eps + j)%N))%Re). { nra. }
                rewrite H21. clear H21.
                assert ((-(a * (m mal init A w (t_eps + j)%N - (A_m + eps_j j eps_0 eps a))))%Re = 
                           (a * ((A_m + eps_j j eps_0 eps a) - m mal init A w (t_eps + j)%N))%Re). { nra. }
                ...
                * apply Rge_le, Rge_minus. apply Rle_ge. apply Rle_trans with A_m.
                  ...
                  ++ assert ((0 < eps_j j eps_0 eps a)%Re).
                     { by apply eps_j_gt_0 with N.  } nra.
                *  rewrite Heqw_ii. apply H9 . apply i_in_incl.
...
++ rewrite (bigD1 (inord (index i incl))) //=. 
  ...
  + rewrite nth_index. 
    - rewrite -RplusE. 
      apply Rge_trans with ((m mal init A w (t_eps +j)%N) * w (t_eps + j)%N (i, i) +
           \sum_(i0 < (size incl - 1).+1 | 
           i0 != inord (index i incl))
              (x mal init A w (t_eps + j)%N (nth i incl i0) *
               w (t_eps + j)%N (i, nth i incl i0))%Re).
            ...
            * assert ((m mal init A w (t_eps + j)%N * w (t_eps + j)%N (i, i) +
              ...
              remember (\sum_(i0 < (size incl - 1).+1 | 
                          ( inord (index i incl) < i0)%N)
                             w (t_eps + j)%N (i, nth i incl i0)) as w_ih.
              assert ((m mal init A w (t_eps+j)%N > A_m - eps)%Re). 
              { apply H11. apply /leP. apply leq_addr. }
              apply Rge_trans with ((1 - a) * m mal init A w (t_eps + j)%N +
                                      a * (A_m + eps_j j eps_0 eps a))%Re.
              - assert (((1 - w_ih) * m mal init A w (t_eps + j)%N +
                              w_ih * (A_m + eps_j j eps_0 eps a))%Re = 
                        (m mal init A w (t_eps + j)%N - w_ih * ( m mal init A w (t_eps + j)%N - (A_m + eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H20.
                assert (((1 - a) * m mal init A w (t_eps + j)%N +
                            a * (A_m + eps_j j eps_0 eps a))%Re = 
                         (m mal init A w (t_eps + j)%N - a * (m mal init A w (t_eps + j)%N - (A_m + eps_j j eps_0 eps a)))%Re).
                { nra. } rewrite H21.
                apply Rplus_ge_compat_l. 
                assert ((- (w_ih * (m mal init A w (t_eps + j)%N - (A_m + eps_j j eps_0 eps a))))%Re = 
                          (w_ih * ((A_m + eps_j j eps_0 eps a) - m mal init A w (t_eps + j)%N))%Re). { nra. }
...
Lemma in_neighbor_subset_incl_last (i:D) (a A_m:R) (t_eps j:nat) (eps eps_0: posreal)
  ...
  {subset (in_neighbor i  :\: 
                X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w) <= 
        [set x | x \in incl] :|: R_i_greater_than mal init A w i (t_eps+j)%N}.
Proof.
intros. rewrite /sub_mem //=. move=> k.
intros. rewrite inE in H1. apply and_2_conv in H1.
destruct H1. rewrite inE in H1. rewrite in_setU. apply /orP.
rewrite inE. 
destruct (Rlt_dec (x mal init A w (t_eps + j)%N k) (A_m + eps_j j eps_0 eps a)).
+ by simpl in H1.
+ simpl in H1.
  assert ((index k (inclusive_neighbor_list i (x mal init A w (t_eps + j))) <= size (inclusive_neighbor_list i (x mal init A w (t_eps + j))) - F - 1)%N \/
           (size (inclusive_neighbor_list i (x mal init A w (t_eps + j))) - F - 1  <= index k (inclusive_neighbor_list i (x mal init A w (t_eps + j))))%N).
  { apply /orP. apply leq_total. }
  destruct H3.
  - left.
    rewrite /incl /incl_neigh_minus_extremes /remove_extremes //=.
    ...
    * apply /andP. split.
      ++ apply /orP. left.
         destruct (Rge_dec (x mal init A w (t_eps + j)%N k) (x mal init A w (t_eps + j)%N i)).
         - by simpl.
         - simpl.
           assert ((x mal init A w (t_eps + j)%N k >= x mal init A w (t_eps + j)%N i)%Re).
  ...
  - rewrite leq_eqVlt in H3.
    assert ((index k (inclusive_neighbor_list i (x mal init A w (t_eps + j))) == (size (inclusive_neighbor_list i (x mal init A w (t_eps + j))) - F - 1)%N)
              \/ (size (inclusive_neighbor_list i (x mal init A w (t_eps + j))) - F - 1 <
                      index k (inclusive_neighbor_list i (x mal init A w (t_eps + j))))%N).
    ...
    * right. rewrite inE. apply /andP. split.
      ++ apply /andP. split.
         - destruct (Rgt_dec (x mal init A w (t_eps + j)%N k) (x mal init A w (t_eps + j)%N i)).
           * by simpl.
           * simpl.
             assert ((x mal init A w (t_eps + j)%N k > x mal init A w (t_eps + j)%N i)%Re).
...
Lemma s_subset_incl_last (i:D) (a A_m:R) (t_eps j:nat) (eps eps_0: posreal) (s: seq D)
  ...
  {subset s <= in_neighbor i  :\: 
                X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w} ->
  {subset s <= [set x | x \in incl] :|: R_i_greater_than mal init A w i (t_eps+j)%N}.
Proof.
intros.
assert ({subset [set x | x \in s] <= [set x | x \in incl] :|: R_i_greater_than mal init A w i (t_eps+j)%N}).
{ apply /subsetP.
  apply (@subset_trans D [set x | x \in s] 
   (in_neighbor i  :\: 
                  X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w)
...
+ apply Rle_ge.
  ...
  - by apply x_bound.
  - assert ((#|R_i_less_than mal init A w i (t_eps + j)%N| <= F)%N).
    { apply R_i_lt_bounded . } rewrite leq_eqVlt in H15.
    assert ( (#|R_i_less_than mal init A w i (t_eps + j)%N| == F)
              \/ (#|R_i_less_than mal init A w i (t_eps + j)%N| < F)%N).
    { by apply /orP. } destruct H16.
    * assert(R_i_less_than mal init A w i (t_eps + j)%N == Adversary A \/
      ...
      ++ assert (exists k:D, k \in (Normal A :&: (R_i_less_than mal init A w  i (t_eps+j)%N))).
        ...
        assert ((p \in Normal A) /\
                     (p \in R_i_less_than mal init A w i (t_eps + j)%N)).
        { by apply /andP. } destruct H19.
        apply Rle_trans with (x mal init A w (t_eps + j)%N p).
        * by apply x_bound.
        * apply R_i_lt_val_rel with i.
    ...
    * (** The last node has the value same as x_i(t) and since i is Normal,
      ...
      apply Rle_trans with (x mal init A w (t_eps + j)%N (nth i (incl_neigh_minus_extremes i (x mal init A w (t_eps + j)%N)) 0)).
      - assert (((x mal init A w (t_eps+j)%N (nth i (incl_neigh_minus_extremes i (x mal init A w (t_eps + j)%N)) 0)) = (x mal init A w (t_eps + j)%N i))%Re).
        { by apply  card_R_i_lt_lt_F_exchange_first. } rewrite H17.
        by apply x_bound.
      - apply first_incl_is_min. rewrite -/incl. apply mem_nth.
        apply ltn_leq_trans with (size incl - 1).+1.
        + by apply ltn_ord.
...
+ rewrite leq_eqVlt in H13.
  ...
  - apply Rle_ge.
    ...
    * rewrite inordK.
      ++ rewrite nth_index.
         - assert (sorted_Dseq (x mal init A w (t_eps + j)%N) incl).
           { apply lemma_1.incl_sorted. } rewrite /sorted_Dseq in H15.
           apply H15.
           * apply i_in_incl.
           * apply mem_nth. apply ltn_leq_trans with (size incl - 1).+1.
...
destruct H.
rewrite inE in H.
destruct ( Rlt_dec (x mal init A w (t_eps + j)%N k)
                (A_m + eps_j j eps_0 eps a)).
+ by simpl in H.
...
destruct H16.
assert (exists k:D, k \in [set x | x \in s] :&: [set x | x \in incl]).
{ apply exists_in_intersection with (R_i_greater_than mal init A w i (t_eps+j)%N) F.
  + assert (#|s| = size s). { by apply /card_uniqP. }
    by rewrite H19.
   ...
   * apply Rle_trans with 
     ...
     ++ assert (( A_m + (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re = 
        ...
        - assert ((w_ik * (A_m + eps_j j eps_0 eps a) +
          ...
          { nra. } rewrite H26.
          apply Rplus_le_compat_l. 
          assert ((- (w_ik * (m mal init A w (t_eps + j)%N - (A_m + eps_j j eps_0 eps a))))%Re = 
                          (w_ik * ((A_m + eps_j j eps_0 eps a) - m mal init A w (t_eps + j)%N))%Re). { nra. }
     ...
     (sum_f_R0
        (fun n0 : nat =>
         w (t_eps + 0)%N
           (i,
           nth i
             (incl_neigh_minus_extremes i (x mal init A w (t_eps + 0)))
             n0))
        (size (incl_neigh_minus_extremes i (x mal init A w (t_eps + 0))) -
         1) = 1%Re)) ->
  (forall t : nat,
    (t >= t_eps)%coq_nat ->
    (m mal init A w t > A_m - eps)%Re /\ (M mal init A w t < A_M + eps)%Re) -> 
                ...
                (F + 1 <=
                   #|in_neighbor i
                     :\: X_M_t_e_i (eps_j 0 eps_0 eps a) A_M
                           (t_eps + 0) mal init A w|)%N ) -> 
  (#|X_M_t_e_i (eps_j 1 eps_0 eps a) A_M (t_eps + 1) mal init A w
   :&: Normal A| <
   #|X_M_t_e_i (eps_j 0 eps_0 eps a) A_M t_eps mal init A w
     :&: Normal A|)%N.
Proof.
intros.
...
+ apply /subsetP. rewrite /sub_mem //=. move => i H9.
  ...
  assert ( (i \in X_M_t_e_i (a * eps_0 - (1 - a) * eps)
             A_M (t_eps + 1) mal init A w) /\ (i \in Normal A)).
  { by apply /andP. } destruct H10 as [H10 H11].
  split.
  - rewrite inE. rewrite inE in H10.
    destruct (Rgt_dec (x mal init A w (t_eps + 1) i)
                (A_M - (a * eps_0 - (1 - a) * eps))).
    * simpl in H10. 
      destruct (Rgt_dec (x mal init A w t_eps i) (A_M - eps_0)).
      ++ by simpl.
      ++ simpl. apply Rnot_gt_le in n.
         assert (( x mal init A w (t_eps + 1) i <= A_M - (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_decreases_1 i a A_m A_M t_eps 0%N #|Normal A| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
+ apply /subsetPn. rewrite /eps_j. destruct H8 as [i H8].
  ...
  - destruct H8 as [H8 H9].
    ...
    * simpl in H8. 
      ...
      ++ simpl.
         assert ((x mal init A w (t_eps + 1) i <= A_M - (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_decreases_2 i a A_m A_M t_eps 0%N #|Normal A| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
+ apply /subsetP. rewrite /sub_mem //=. move => i H9.
  ...
  split.
  - rewrite inE. rewrite inE in H10.
    destruct (Rlt_dec (x mal init A w (t_eps + 1) i)
                (A_m + (a * eps_0 - (1 - a) * eps))).
    * simpl in H10. 
      destruct (Rlt_dec (x mal init A w t_eps i) (A_m + eps_0)).
      ++ by simpl.
      ++ simpl. apply Rnot_lt_ge in n.
         assert (( x mal init A w (t_eps + 1) i >= A_m + (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_increases_1 i a A_m A_M t_eps 0%N #|Normal A| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
+ apply /subsetPn. rewrite /eps_j. destruct H8 as [i H8].
  ...
  - destruct H8 as [H8 H9].
    ...
    * simpl in H8. 
      ...
      ++ simpl.
         assert ((x mal init A w (t_eps + 1) i >= A_m + (a * eps_0 - (1 - a) * eps))%Re).
         { apply (@x_val_increases_2 i a A_m A_M t_eps 0%N #|Normal A| eps eps_0).
           + by [].
           + by [].
           + by [].
           + by [].
...
Lemma normal_node_cond (t_eps: nat) (A_M A_m:R) (eps_0 eps:posreal)
  ...
  (A_M - eps_0 > A_m + eps_0)%Re ->
  (F + 1 <=
     #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w)
         (F + 1)| +
     #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w)
         (F + 1) |)%N ->
  exists i:D, (i \in X_M_t_e_i eps_0 A_M t_eps mal init A w /\ i \in Normal A /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_M_t_e_i eps_0 A_M t_eps mal init A w|)%N) \/
               (i \in X_m_t_e_i eps_0 A_m t_eps mal init A w /\ i \in Normal A /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_m_t_e_i eps_0 A_m t_eps mal init A w|)%N).
Proof.
intros.
assert ( #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)|  = 
           (#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1) :&: Normal A | +
             #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1) :\: Normal A|)%N).
{ by rewrite cardsID. }
assert ( #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)| = 
        (#|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1) :&: Normal A | +
             #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1) :\: Normal A|)%N).
...
assert ((#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)
           :&: Normal A| +
         #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)
           :\: Normal A| +
         (#|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
            :&: Normal A| +
          #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
            :\: Normal A|))%N = 
      ((#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)
           :&: Normal A| + #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
            :&: Normal A|) + 
      ( #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)
           :\: Normal A| + #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
            :\: Normal A|))%N).
{ rewrite !addnA. by rewrite sum_eq. } rewrite H4 in H1. clear H4.
apply (@add_max_bound (#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)
       :&: Normal A| +
     #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
       :&: Normal A|)%N   (#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)
        :\: Normal A| +
      #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
        :\: Normal A|)%N F) in H1.
+ assert (exists i:D, 
          i \in ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)
       :&: Normal A) :|:
      (Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
       :&: Normal A))).
  { apply /card_gt0P. rewrite cardsU.
    assert ((#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1) :&: Normal A
               :&: (Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)
                    :&: Normal A)|)%N = 0%N).
    { rewrite setIACA. rewrite setIid.
      ...
      rewrite inE.
      assert ((i \in Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)) = 
                ((i \in X_m_t_e_i eps_0 A_m t_eps mal init A w) && 
                  (F + 1 <=
                    #|in_neighbor i :\: X_m_t_e_i eps_0 A_m t_eps mal init A w|)%N)).
      { by rewrite inE. } rewrite H4.
      rewrite and_distr. apply /nandP. left.
      apply /nandP.
      assert ([disjoint (X_M_t_e_i eps_0 A_M t_eps mal init A w) & (X_m_t_e_i eps_0 A_m t_eps mal init A w)]).
      { by apply X_M_X_m_disjoint. }
      apply (@implies_simp (i \in (X_M_t_e_i eps_0 A_M t_eps mal init A w))
                (i \notin X_m_t_e_i eps_0 A_m t_eps mal init A w)).
      intros.
      assert ((i \in (X_m_t_e_i eps_0 A_m t_eps mal init A w) = false) ->
             i \notin (X_m_t_e_i eps_0 A_m t_eps mal init A w)). 
      { intros. by apply ssrbool.negbT. }
      apply H7. 
      by apply (@disjointFr D (X_M_t_e_i eps_0 A_M t_eps mal init A w) (X_m_t_e_i eps_0 A_m t_eps mal init A w) i). 
    } by rewrite H4 subn0.
  } destruct H4 as [ i H4].
  exists i.
  assert ( i \in (Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w)
                    (F + 1) :&: Normal A) \/ 
           i \in (Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w)
                    (F + 1) :&: Normal A)).
  ...
  - left.
    ...
    rewrite inE in H6.
    assert ((i \in X_M_t_e_i eps_0 A_M t_eps mal init A w) /\
               (F + 1 <=
                #|in_neighbor i :\: X_M_t_e_i eps_0 A_M t_eps mal init A w|)%N).
...
+ apply adversary_max.
  ...
  - apply eq_finset. unfold eqfun. move=>i.
    ...
    apply (@disjointFr D (X_M_t_e_i eps_0 A_M t_eps mal init A w) (X_m_t_e_i eps_0 A_m t_eps mal init A w) i).
    * by apply X_M_X_m_disjoint.
    * by []. 
Qed.
...
assert ( exists i:D, i \in (X_M_t_e_i eps_0 A_M t_eps mal init A w) /\ i \in Normal A).
...
Lemma normal_node_cond1_implies (t_eps: nat) (A_M A_m:R) (eps_0 eps:posreal)
  ...
  (0 < F + 1 <= #|D|)%N ->
   is_lim_seq [eta M mal init A w] A_M -> 
  #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)| ==
     #|X_M_t_e_i eps_0 A_M t_eps mal init A w| ->
  exists i:D, (i \in X_M_t_e_i eps_0 A_M t_eps mal init A w /\ i \in Normal A /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_M_t_e_i eps_0 A_M t_eps mal init A w|)%N) \/
...
Proof.
intros.
assert ( exists i:D, i \in (X_M_t_e_i eps_0 A_M t_eps mal init A w) /\
                        i \in Normal A).
...
+ split.
  ...
  - rewrite eqn_leq in H3. apply split_ineq in H3.
    ...
    assert ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)) \subset 
              (X_M_t_e_i eps_0 A_M t_eps mal init A w)).
    ...
    assert ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1) \subset
               X_M_t_e_i eps_0 A_M t_eps mal init A w) &&  
            ((#|X_M_t_e_i eps_0 A_M t_eps mal init A w| <=
                #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)|)%N)).
    { apply /andP. by split. }
    assert (Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1) == 
                  X_M_t_e_i eps_0 A_M t_eps mal init A w).
    { by rewrite eqEcard. }
    ...
    assert ((Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1) \subset
                  X_M_t_e_i eps_0 A_M t_eps mal init A w) /\
                 (X_M_t_e_i eps_0 A_M t_eps mal init A w\subset
                  Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1))).
    { by apply /andP. } destruct H10 as [H10 H11].
    rewrite subsets_disjoint in H11.
    apply (@disjointFr D (X_M_t_e_i eps_0 A_M t_eps mal init A w)
              (~: (Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1))) i) in H11.
...
+ rewrite /F_total_malicious in H. destruct H. 
...
Lemma exists_at_jp1_implies_at_j (t_eps j N: nat) (a A_m A_M :R) (eps eps_0: posreal)
  ...
  (forall i:D, (*i \in Normal ->  *)
    let incl :=
       ...
       (forall k : D,
        k \in incl ->
        (a <= w (t_eps + j)%N (i, k))%Re) /\
       sum_f_R0
         (fun n : nat =>
          w (t_eps + j)%N (i, nth i incl n))
         (size incl - 1) = 1%Re) -> 
  (forall t : nat,
        (t >= t_eps)%coq_nat ->
        (m mal init A w t > A_m - eps)%Re /\
        (M mal init A w t < A_M + eps)%Re) ->
   (0 <
       #|X_M_t_e_i (eps_j j.+1 eps_0 eps a) A_M
           (t_eps + j.+1) mal init A w:&: Normal A|)%N ->
  (0 < #|X_M_t_e_i (eps_j j eps_0 eps a)
                             A_M (t_eps + j)%N mal init A w :&: Normal A|)%N.
Proof.
intros.
...
apply /andP. split.
+ rewrite inE. rewrite inE in H13.
  destruct (Rlt_dec (x mal init A w (t_eps + j.+1) i)
                (A_m + eps_j j.+1 eps_0 eps a)).
  - simpl in H13.
    destruct (Rlt_dec (x mal init A w (t_eps + j)%N i)
                  (A_m + eps_j j eps_0 eps a)).
    * by simpl.
    * simpl. apply Rnot_lt_ge in n.
      assert ((x mal init A w (t_eps + j.+1) i >=
...
Lemma max_bound_var (A_M:R) (t:nat) (mal : nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  F_totally_bounded A ->
  (0 < F + 1 <= #|D|)%N ->
  (forall (i:D), 
    i \in Normal A -> (x mal init A w t i < A_M)%Re) ->
   (M mal init A w t < A_M)%Re.
Proof.
intros. 
rewrite /M. apply bigmax_lt.
...
Lemma X_M_card_le (t_eps j N: nat) (a A_M A_m:R) (eps_0 eps:posreal)
  ...
  (#|X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w
   :&: Normal A| <=
 #|X_M_t_e_i (eps_j j.-1 eps_0 eps a) A_M (t_eps + j.-1) mal init A w
   :&: Normal A|)%N.
Proof.
intros.
...
assert ((k \in X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w) /\
          (k \in Normal A)).
...
+ rewrite inE in H12. rewrite inE.
  destruct (Rgt_dec (x mal init A w (t_eps + j)%N k) (A_M - eps_j j eps_0 eps a)).
  - simpl.
    destruct (Rgt_dec (x mal init A w (t_eps + j.-1) k) (A_M - eps_j j.-1 eps_0 eps a)).
    * by simpl.
    * simpl. apply Rnot_gt_le in n.
      assert ((x mal init A w (t_eps + j)%N k <= A_M - eps_j j eps_0 eps a)%Re).
      { assert (j = (j-1).+1). { rewrite subn1. by rewrite prednK. }
        rewrite H14.
...
+ rewrite inE in H12. rewrite inE.
  destruct (Rlt_dec (x mal init A w (t_eps + j)%N k) (A_m + eps_j j eps_0 eps a)).
  - simpl.
    destruct (Rlt_dec (x mal init A w (t_eps + j.-1) k) (A_m + eps_j j.-1 eps_0 eps a)).
    * by simpl.
    * simpl. apply Rnot_gt_le in n.
      assert ((x mal init A w(t_eps + j)%N k >= A_m + eps_j j eps_0 eps a)%Re).
      { assert (j = (j-1).+1). { rewrite subn1. by rewrite prednK. }
  ...
  (exists i:D, (i \in X_M_t_e_i eps_0 A_M t_eps mal init A w /\ i \in Normal A /\
               ...
               (i \in X_m_t_e_i eps_0 A_m t_eps mal init A w /\ i \in Normal A /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_m_t_e_i eps_0 A_m t_eps mal init A w|)%N)) ->
  (#|X_M_t_e_i (eps_j 0 eps_0 eps a) A_M (t_eps + 0) mal init A w
   :&: Normal A| +
 #|X_m_t_e_i (eps_j 0 eps_0 eps a) A_m (t_eps + 0) mal init A w
   :&: Normal A| <= N)%N ->
  (#|X_M_t_e_i (eps_j 1 eps_0 eps a) A_M (t_eps + 1) mal init A w
...
+ destruct H10. destruct H12.
  ...
  - rewrite ltn_add2r. apply card_decreases. exists i.
    ...
    * rewrite inE. apply /nandP. left. rewrite inE.
      ...
      destruct (Rgt_dec (x mal init A w t_eps i) (A_M - eps_0)).
      ++ simpl in H11.
         destruct (Rgt_dec (x mal init A w (t_eps + 1) i) (A_M - eps_j 1 eps_0 eps a)).
         + simpl.
           assert ((x mal init A w (t_eps + 1) i <=  A_M - eps_j 1 eps_0 eps a)%Re).
    ...
    * apply /subsetP. rewrite /sub_mem //=. move=> p H14.
         ...
         assert ((p \in X_M_t_e_i (a * eps_j 0 eps_0 eps a - (1 - a) * eps) A_M
                            (t_eps + 1) mal init A w) /\ (p \in Normal A)).
         { simpl. by apply /andP. } destruct H15.
         rewrite in_setI. apply /andP. split.
         * rewrite inE. rewrite inE in H15.
           rewrite addn0.
           destruct (Rgt_dec (x mal init A w (t_eps + 1) p)
                          (A_M - (a * eps_j 0 eps_0 eps a  - (1 - a) * eps))).
           ++ simpl in H15.
              destruct (Rgt_dec (x mal init A w (t_eps) p)
                                (A_M - eps_0 )).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x mal init A w (t_eps + 1) p <= A_M -
...
+ destruct H10. destruct H12.
  ...
  - rewrite ltn_add2l. apply card_decreases. exists i.
    ...
    * rewrite inE. apply /nandP. left. rewrite inE.
      ...
      destruct (Rlt_dec (x mal init A w t_eps i) (A_m +eps_0)).
      ++ simpl in H10.
         destruct (Rlt_dec (x mal init A w (t_eps + 1) i) (A_m + eps_j 1 eps_0 eps a)).
         + simpl. 
           assert ((x mal init A w (t_eps + 1) i >=  A_m + eps_j 1 eps_0 eps a)%Re).
    ...
    * apply /subsetP. rewrite /sub_mem //=. move=> p H14.
         ...
         * rewrite inE. rewrite inE in H15.
           rewrite addn0.
           destruct (Rlt_dec (x mal init A w (t_eps + 1) p)
                          (A_m + (a * eps_j 0 eps_0 eps a  - (1 - a) * eps))).
           ++ simpl in H15.
              destruct (Rlt_dec (x mal init A w (t_eps) p)
                                (A_m + eps_0 )).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x mal init A w (t_eps + 1) p >= A_m +
...
intros.
rewrite -cardsUI.
assert (#|X_M_t_e_i (eps_j 0 eps_0 eps a) A_M (t_eps + 0) mal init A w
         :&: Normal A
         :&: (X_m_t_e_i (eps_j 0 eps_0 eps a) A_m (t_eps + 0) mal init A w
              :&: Normal A)| = 0%N).
{ rewrite -setIACA setIid.
  assert (X_M_t_e_i (eps_j 0 eps_0 eps a) A_M (t_eps + 0) mal init A w
            :&: X_m_t_e_i (eps_j 0 eps_0 eps a) A_m (t_eps + 0) mal init A w = set0).
  ...
  (forall t : nat,
   ...
   (forall (i:D)(t:nat), (*i \in Normal -> *) 
    ...
    (exists i : D,
      i \in X_M_t_e_i eps_0 A_M t_eps mal init A w /\
      i \in Normal A /\
      (F + 1 <= #|in_neighbor i :\: X_M_t_e_i eps_0 A_M t_eps mal init A w|)%N \/
      i \in X_m_t_e_i eps_0 A_m t_eps  mal init A w /\
      i \in Normal A /\
      (F + 1 <= #|in_neighbor i :\: X_m_t_e_i eps_0 A_m t_eps mal init A w|)%N) ->
     (forall j:nat, (0<j)%N -> (j<=N)%N ->
                (eps_j j eps_0 eps a < eps_j (j-1)%N eps_0 eps a)%Re ->   
                 ( ( (0 < #|(X_M_t_e_i (eps_j j eps_0 eps a ) A_M (t_eps+j)%N) mal init A w :&: Normal A|)%N ->
                      (#|(X_M_t_e_i (eps_j j eps_0 eps a ) A_M (t_eps+j)%N) mal init A w :&: Normal A| <
                  #| (X_M_t_e_i (eps_j (j-1)%N eps_0 eps a)%N A_M (t_eps+(j-1))%N) mal init A w :&: Normal A|)%N) \/
                  ( (0 < #|(X_m_t_e_i (eps_j j eps_0 eps a ) A_m (t_eps+j)%N) mal init A w :&: Normal A|)%N ->
                    (#|(X_m_t_e_i (eps_j j eps_0 eps a ) A_m (t_eps+j)%N) mal init A w :&: Normal A| <
                      #| (X_m_t_e_i (eps_j (j-1)%N eps_0 eps a)%N A_m (t_eps+(j-1))%N) mal init A w :&: Normal A|)%N))) ->
    (exists t : nat, (M mal init A w t < A_M)%Re \/ (m mal init A w t > A_m)%Re).
Proof.
intros.
assert (exists T:nat,(T<=N)%N /\ 
          (#|(X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T)%N) mal init A w :&: Normal A| = 0%N \/
           #|(X_m_t_e_i (eps_j T eps_0 eps a) A_m (t_eps + T)%N) mal init A w :&: Normal A| = 0%N)).
{ apply sj_ind_var.
...
+ left. apply cards0_eq in H14.
  assert (X_M_t_e_i (eps_j T eps_0 eps a) A_M
            (t_eps + T)  mal init A w :&: Normal A == set0).
  { by apply /eqP. } rewrite setI_eq0 in H15.
  assert ((forall i : D, i \in Normal A -> (x mal init A w (t_eps+T)%N i < A_M)%Re) -> 
              (M mal init A w (t_eps + T) < A_M)%Re).
  { destruct H. by apply max_bound_var. } apply H16. intros. 
  assert ([disjoint
          X_M_t_e_i (eps_j T eps_0 eps a) A_M
            (t_eps + T)  mal init A w & Normal A] ->
           i \in Normal A ->
           i \notin (X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T) mal init A w )).
  { intros. 
    assert ((i \in (X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T) mal init A w ) = false) ->
               i \notin (X_M_t_e_i (eps_j T eps_0 eps a) A_M (t_eps + T) mal init A w )). 
    { intros. by apply ssrbool.negbT. } 
    apply H20. move: H18 H19. apply disjointFl.
  } specialize (H18 H15 H17).
  rewrite inE in H18.
  destruct (Rgt_dec (x mal init A w (t_eps + T) i)
...
+ right. apply cards0_eq in H14.
  ...
  rewrite inE in H18.
  destruct (Rlt_dec (x mal init A w (t_eps + T) i)
         (A_m + eps_j T eps_0 eps a)).
  + by simpl in H18. 
  + intros. simpl in H18. apply Rnot_gt_le in n.
  ...
  (mal : nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R): 
   ...
   is_lim_seq [eta M mal init A w] A_M ->
    ...
    exists i:D, i \in (X_M_t_e_i (eps_j j eps_0 eps a) A_M
                (t_eps + j)%N mal init A w ) /\ i \in Normal A.
Proof.
intros. 
assert (M mal init A w (t_eps+j)%N \in map (fun i:D => x mal init A w  (t_eps +j) i) (enum (Normal A))).
{ rewrite /M . 
  apply (@bigmaxr_mem _ 0%Re (map (fun i:D => x mal init A w (t_eps+j)%N i) (enum (Normal A)))).
  rewrite size_map. destruct H. by apply size_normal_gt_0.
} 
assert (exists2 i:D, i \in (enum (Normal A)) &
            (M mal init A w (t_eps+j)%N = (fun i:D => x mal init A w (t_eps+j)%N i) i)).
{ by apply /mapP. }
destruct H8 as [i H8]. exists i.
split.
+ rewrite inE. rewrite -H9.
  destruct (Rgt_dec (M mal init A w (t_eps+j)%N) (A_M - eps_j j eps_0 eps a)).
  - by simpl.
  - simpl.  
    assert ((A_M - eps_j j eps_0 eps a < M mal init A w (t_eps+j)%N)%Re).
    { apply Rlt_le_trans with (A_M - 0)%Re.
      + apply Rplus_lt_compat_l. apply Ropp_gt_lt_contravar.
...
Proof.
intros. 
assert ((-(m mal init A w (t_eps+j)%N)) \in map (fun i:D => - x mal init A w (t_eps+j)%N i) (enum (Normal A))).
{ rewrite /m. rewrite -RoppE. rewrite Ropp_involutive.
  apply (@bigmaxr_mem _ 0%Re (map (fun i:D => - x mal init A w (t_eps+j)%N i) (enum (Normal A)))).
  rewrite size_map. destruct H. by apply size_normal_gt_0.
} 
...
split.
+ rewrite inE.
  assert (m mal init A w (t_eps +j)= x mal init A w (t_eps +j) i). 
  { rewrite -[LHS]Ropp_involutive. rewrite !RoppE. rewrite H9.
    by rewrite -!RoppE Ropp_involutive.
  } rewrite -H10.
  destruct (Rlt_dec (m mal init A w (t_eps+j)%N) (A_m + eps_j j eps_0 eps a)).
  - by simpl.
  - simpl. 
    assert ((m mal init A w (t_eps +j)  < A_m + eps_j j eps_0 eps a)%Re).
    { apply Rle_lt_trans with (A_m + 0)%Re.
      + assert ((A_m + 0)%Re = A_m). { nra. }
...
Lemma X_M_X_m_disjoint_at_j (mal : nat -> D ->R) (init: D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  forall (t_eps j:nat) (a A_M A_m :R) (eps_0 eps :posreal),
  (A_M - (eps_j j eps_0 eps a) > A_m + (eps_j j eps_0 eps a))%Re ->
  [disjoint (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) & 
    (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w )].
Proof.
intros. rewrite -setI_eq0. apply /eqP.
apply eq_finset. unfold eqfun. move=>i.
...
rewrite !in_set. apply /andP. 
apply /andP. apply /nandP.
destruct (Rgt_dec (x mal init A w (t_eps+j)%N i) (A_M - (eps_j j eps_0 eps a))).
+ simpl. 
  destruct (Rlt_dec (x mal init A w (t_eps+j)%N i) (A_m + (eps_j j eps_0 eps a))).
  - simpl. left.
    assert ((x mal init A w (t_eps+j)%N i > A_m + (eps_j j eps_0 eps a))%Re).
    { apply Rgt_trans with (A_M - (eps_j j eps_0 eps a))%Re; by []. }
    assert ((x mal init A w (t_eps+j)%N i > A_m + (eps_j j eps_0 eps a))%Re -> 
            ~(x mal init A w (t_eps+j)%N i < A_m + (eps_j j eps_0 eps a))%Re). { nra. }
    specialize (H1 H0).
    contradiction.
  - simpl. by right.
+ simpl. by left.
...
- rewrite cardsCs.
  ...
  rewrite ltn_subrL. apply /andP. split.
  * rewrite card_gt0. apply /set0Pn.
    exists (nth key (enum (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w)) 0%N).
    rewrite inE -disjoints1.
    apply single_element_disjoint.
    ++ by [].
    ++ rewrite disjoint_sym.
...
Lemma X_m_mem_j (mal : nat -> D -> R) (init : D -> R) (A: D -> bool) (w: nat -> D * D -> R):
  forall (t_eps j:nat) (a A_M A_m :R) (eps_0 eps:posreal),
  (0 < #|D|)%N ->  (A_M - (eps_j j eps_0 eps a) > A_m + (eps_j j eps_0 eps a))%Re -> 
  (0 < #|X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w|)%N -> 
  X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w \proper Vertex.
Proof.
intros.
rewrite properEcard.
apply /andP. split.
...
- rewrite cardsCs.
  ...
  * rewrite card_gt0. apply /set0Pn.
    ...
    apply single_element_disjoint.
    ++ by [].
    ++ by apply (@X_M_X_m_disjoint_at_j mal init A w t_eps j a A_M A_m eps_0 eps).
  * apply H.
Qed.
...
Lemma normal_node_cond_at_j (t_eps j N: nat) (a A_M A_m:R) (eps_0 eps:posreal)
  ...
  (eps < a ^ N / (1 - a ^ N) * eps_0)%Re ->
  (F + 1 <=
     #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w)
         (F + 1)| +
     #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w)
         (F + 1)|)%N ->
  exists i:D, (i \in X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w /\ i \in Normal A /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w|)%N) \/
               (i \in X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w  /\ i \in Normal A /\
                   (F + 1 <=
                           #|in_neighbor i
                             :\: X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w|)%N).
Proof.
intros.
assert ( #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)|  = 
           (#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1) :&: Normal A | +
             #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1) :\: Normal A|)%N).
{ by rewrite cardsID. }
assert ( #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)| = 
        (#|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1) :&: Normal A | +
             #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1) :\: Normal A|)%N).
...
assert ((#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)
           :&: Normal A| +
         #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)
           :\: Normal A| +
         (#|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
            :&: Normal A| +
          #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
            :\: Normal A|))%N = 
      ((#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)
           :&: Normal A| + #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
            :&: Normal A|) + 
      ( #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)
           :\: Normal A| + #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
            :\: Normal A|))%N).
{ rewrite !addnA. by rewrite sum_eq. } rewrite H9 in H6. clear H9.
apply (@add_max_bound (#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)
       :&: Normal A| +
     #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
       :&: Normal A|)%N   (#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)
        :\: Normal A| +
      #|Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
        :\: Normal A|)%N F) in H6.
+ assert (exists i:D, 
          i \in ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)
       :&: Normal A) :|:
      (Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
       :&: Normal A))).
  { apply /card_gt0P. rewrite cardsU.
    assert ((#|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1) :&: Normal A
               :&: (Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)
                    :&: Normal A)|)%N = 0%N).
    { rewrite setIACA. rewrite setIid.
      apply /eqP. rewrite cards_eq0. apply /eqP. apply eq_finset.
      ...
      apply /nandP. left. rewrite inE. 
      rewrite inE.
      assert ((i \in Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) (F + 1)) = 
                ((i \in X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) && 
                  (F + 1 <=
                    #|in_neighbor i :\: X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w |)%N)).
      { by rewrite inE. } rewrite H9.
      rewrite and_distr. apply /nandP. left.
      apply /nandP.
      assert ([disjoint (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) & (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w)]).
      { by apply X_M_X_m_disjoint_at_j. }
      apply (@implies_simp (i \in (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w))
                (i \notin X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w)).
      intros.
      assert ((i \in (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) = false) ->
             i \notin (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w)). 
      { intros. by apply ssrbool.negbT. }
      apply H12. 
      by apply (@disjointFr D (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) i). 
    } by rewrite H9 subn0.
  } destruct H9 as [ i H9].
  exists i.
  assert ( i \in (Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w)
                    (F + 1) :&: Normal A) \/ 
           i \in (Xi_S_r (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w)
                    (F + 1) :&: Normal A)).
  { apply /orP. by rewrite -in_setU. }
  destruct H10.
  - left.
    ...
    rewrite inE in H11.
    assert ((i \in X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) /\
               (F + 1 <=
                #|in_neighbor i :\: X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w|)%N).
...
+ apply adversary_max.
  ...
  - apply eq_finset. unfold eqfun. move=>i.
    ...
    apply /nandP. apply implies_simp.
    intros. apply ssrbool.negbT.
    apply (@disjointFr D (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps+j)%N mal init A w) i).
    * by apply X_M_X_m_disjoint_at_j.
    * by []. 
Qed.
...
Proof. 
intros.
assert ( exists i:D, i \in (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w) /\
                        i \in Normal A).
...
+ split.
  ...
  - rewrite eqn_leq in H8. apply split_ineq in H8.
    ...
    assert ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)) \subset 
              (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w)).
    ...
    assert ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1) \subset
               X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) && 
            ((#|X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w| <=
                #|Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1)|)%N)).
    { apply /andP. by split. }
    assert (Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1) == 
                  X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w).
    { by rewrite eqEcard. }
    ...
    assert ((Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1) \subset
                  X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) /\
                 (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w \subset
                  Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1))).
    { by apply /andP. } destruct H15 as [H15 H16].
    rewrite subsets_disjoint in H16.
    apply (@disjointFr D (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w)
              (~: (Xi_S_r (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps+j)%N mal init A w) (F + 1))) i) in H16.
...
Proof.
intros.
assert ((0 < #|X_M_t_e_i eps_0 A_M t_eps mal init A w|)%N). { by apply X_M_card_gt_0. }
assert ((0 < #|X_m_t_e_i eps_0 A_m t_eps mal init A w|)%N). { by apply X_m_card_gt_0. }
assert (X_M_t_e_i eps_0 A_M t_eps mal init A w \proper Vertex).
{ apply (@X_M_mem mal init A w t_eps A_M A_m eps_0).
  ...
  - assert (j=0%N). { rewrite eq_sym in H23. by apply /eqP. }
   ...
   - specialize (IHj H23). 
     ...
     assert ((0 < #|X_M_t_e_i (eps_j j eps_0 eps a) A_M
                (t_eps + j)%N mal init A w:&: Normal A|)%N).
     { apply /card_gt0P. 
       assert (exists i : D, 
                  (i \in X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w) /\
                  ( i \in Normal A)).
     ...
     destruct H5.
     specialize (H27 H6).
     specialize (H27 (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w)
                     (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w)).
     ...
     { split.
       + apply X_M_mem_j with A_m.
         ...
         - apply /card_gt0P.
           assert ( exists i:D, (i \in X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w) /\
                        (i \in Normal A)).
           { assert (exists i:D, i \in (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w) :&: Normal A).
    ...
    { apply r_s_robustness_cond_implies_at_j with N.
      ...
      + destruct ((#|Xi_S_r
           (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w)
           (F + 1)| ==
       #|X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w|)) eqn:E.
        by [].
        destruct ((#|Xi_S_r
              (X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w)
              (F + 1)| ==
          #|X_m_t_e_i (eps_j j eps_0 eps a) A_m (t_eps + j)%N mal init A w|)) eqn:E0.
         by [].
        destruct ((F + 1 <=
          #|Xi_S_r
              (X_M_t_e_i (eps_j j eps_0 eps a) A_M (t_eps + j)%N mal init A w)
      ...
      - exists k. split.
        ...
        * rewrite inE. apply /nandP. left.
          ...
          destruct (Rgt_dec (x mal init A w (t_eps + j)%N k)
                            (A_M - eps_j j eps_0 eps a)).
          ++ simpl in H31. 
             destruct (Rgt_dec (x mal init A w (t_eps + j.+1) k)
                              (A_M - eps_j j.+1 eps_0 eps a)).
             - simpl. 
               assert ((x mal init A w (t_eps + j.+1) k <= A_M -
       ...
       - apply /subsetP. rewrite /sub_mem //=. move=> p H36.
         ...
         assert ((p \in X_M_t_e_i (a * eps_j j eps_0 eps a - (1 - a) * eps) A_M
                            (t_eps + j.+1) mal init A w) /\ (p \in Normal A)).
         { by apply /andP. } destruct H37.
         rewrite in_setI. apply /andP. split.
         * rewrite inE. rewrite inE in H37.
           destruct (Rgt_dec (x mal init A w (t_eps + j.+1) p)
                          (A_M - (a * eps_j j eps_0 eps a - (1 - a) * eps))).
           ++ simpl in H37.  
              destruct (Rgt_dec (x mal init A w (t_eps + j)%N p)
                                (A_M - eps_j j eps_0 eps a)).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x mal init  A w (t_eps + j.+1)%N p <= A_M -
                                  (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re).
                { apply x_val_decreases_1 with A_m N.
                  + by [].
                  + by []. 
     ...
     * right. destruct H31. destruct H32.
        ...
        * rewrite inE. apply /nandP. left.
          ...
          destruct (Rlt_dec (x mal init A w (t_eps + j)%N k)
                            (A_m + eps_j j eps_0 eps a)).
          ++ simpl in H31. 
             destruct (Rlt_dec (x mal init A w (t_eps + j.+1) k)
                              (A_m + eps_j j.+1 eps_0 eps a)).
             - simpl. 
               assert ((x mal init A w  (t_eps + j.+1) k >= A_m +
                                  (a * eps_j j eps_0 eps a -(1 - a) * eps))%Re).
       ...
       - apply /subsetP. rewrite /sub_mem //=. move=> p H36.
         ...
         rewrite in_setI. apply /andP. split.
         * rewrite inE. rewrite inE in H37.
           destruct (Rlt_dec (x mal init A w (t_eps + j.+1) p)
                          (A_m + (a * eps_j j eps_0 eps a - (1 - a) * eps))).
           ++ simpl in H37.  
              destruct (Rlt_dec (x mal init A w (t_eps + j)%N p)
                                (A_m + eps_j j eps_0 eps a)).
              - by simpl. 
              - simpl. intros. apply Rnot_gt_le in n.
                assert ((x mal init A w (t_eps + j.+1) p >= A_m +
                                  (a * eps_j j eps_0 eps a - (1 - a) * eps))%Re).
                { apply x_val_increases_1 with A_M N.
                  + by [].
                  + by []. 
...
+ assert ((forall t:nat, (M mal init A w t.+1 <= M mal init A w t)%Re) -> 
  ...
  { intros. 
    ...
    + apply Rabs_def2 in H5. destruct H5. 
      apply Rlt_gt.
      assert ( m mal init A w t = (A_m + (m mal init A w t - A_m))%Re). { nra. }
   ...
   **)
   assert (exists t:nat,  (M mal init A w t < A_M)%Re \/ (m mal init A w t > A_m)%Re).
   { assert (exists T:nat, (T<=N)%N).
     { exists (N)%N. by []. }
     ...
     { specialize (H2 (X_M_t_e_i eps_0 A_M t_eps mal init A w) (X_m_t_e_i eps_0 A_m t_eps mal init A w)).
       assert([|| #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)| ==
       #|X_M_t_e_i eps_0 A_M t_eps mal init A w|,
       #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)| ==
       #|X_m_t_e_i eps_0 A_m t_eps mal init A w|
       ...
       #|X_M_t_e_i eps_0 A_M t_eps mal init A w|)
       || (#|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)| ==
       #|X_m_t_e_i eps_0 A_m t_eps mal init A w|) || (F + 1 <=
       #|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)| +
       #|Xi_S_r (X_m_t_e_i eps_0 A_m t_eps mal init A w) (F + 1)|)%N).
       {by destruct (#|Xi_S_r (X_M_t_e_i eps_0 A_M t_eps mal init A w) (F + 1)| ==
       #|X_M_t_e_i eps_0 A_M t_eps mal init A w|).
       }
    ...
    { rewrite HeqN. apply r_s_robustness_implies.
      + by [].
      + rewrite /wts_well_behaved. exists a. by split.
      + by [].
      + by [].
      + by [].
      + by [].
    ...
    { rewrite HeqN. apply normal_bound_exists.
      ...
      + apply r_s_robustness_cond_implies.
        - by [].
        - by [].
        - rewrite /wts_well_behaved. by exists a.
        - by [].
        - by [].
        - by [].
        - by [].
        - apply H16.
          * assert ((0 < #|X_M_t_e_i eps_0 A_M t_eps mal init A w|)%N). 
            { apply X_M_card_gt_0.
              + by [].
              + by [].
              + rewrite /wts_well_behaved. by exists a.
   ...
   assert (forall t : nat, (m mal init A w t <= A_m)%Re). 
   { by apply m_A_m_lower_bound. }
   specialize (H11 t). specialize (H15 t).
   assert ((M mal init A w t >= A_M)%Re /\ (m mal init A w t <= A_m)%Re ).
   { split.
     + by apply Rle_ge.
     + by [].
   }
